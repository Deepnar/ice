...
MODEL_NAME = "qwen-labeler_v2"
CONCURRENT_REQUESTS = 4  # Sweet spot for Ollama parallel processing
...
async def label_prompt_async(semaphore: asyncio.Semaphore, item: dict, outfile, failed_file, pbar):
    async with semaphore:
        ...
        try:
            ...
            tqdm.write(f"⚠️ FAILED ID {item['id']}: {e}")
            failed_file.write(json.dumps({"id": item["id"], "prompt": prompt_text, "error": str(e)}) + "\n")
            failed_file.flush()
            pbar.update(1)
            return
...
async def main():
    INPUT_PATH = "/home/deepnar/Programs/ice/data/labeled/dataset_cleaned.jsonl" 
