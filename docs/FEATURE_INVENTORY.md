...
| Foreground generation transport | `src/api/foreground_transport.py`, `src/api/main.py` | G32(b) | Local Ollama uses native `/api/chat` and converts complete NDJSON answers to the client SSE contract; externally configured models keep OpenAI-compatible SSE. Thinking is not answer text. An incomplete stream is reported and is not stored. | — | — | YES |
...
| Primary-model timeout fallback | `src/api/main.py` | G5/G32(b) | On connect/read timeout or a remote protocol error, emits `degraded` and retries the local registry fallback model. A completed fallback supplies the model identity for post-flight; partial or failed output is not stored. | `foreground_read_timeout_seconds` | `120.0` seconds read timeout | YES |
...
| Token-prediction reconciliation | `src/api/main.py`, `src/api/foreground_transport.py` | C16/G32(b) | Native Ollama's final counts or compatible SSE usage are compared with ICE's prompt prediction when available. Logged, never enforced. | `token_usage_reconciliation` | `True` | YES |
| `num_ctx` request | `src/api/foreground_transport.py` | C16/G32(b) | Local native requests explicitly ask for the guarded prompt plus generation reserve (`fit`), or the capped serving window (`max`). External OpenAI-compatible requests receive no Ollama-native options. | `ollama_send_num_ctx`, `ollama_num_ctx_mode`, `ollama_num_ctx_max` | `True`, `fit`, `32768` | **YES for local Ollama** |
| Post-flight turn storage | `src/api/main.py` | G32(b) | After a complete, nonempty response, embeds the prompt and writes the turn with an idempotency key. Incomplete and failed generations are excluded. Runs as a FastAPI BackgroundTask. | — | — | YES |
...
| Shared conversation pressure | `src/memory/conversation_stats.py:11` | G29/B2 | One query gives chat and an explicitly scoped pull the same warm-turn count and approximate history tokens. This estimate only informs B2 and dynamic retrieval budgeting; prompt spending is counted separately. | — | — | YES (when a conversation is known) |
...
| Evolving conversation notes | `src/api/prompt_assembler.py::conversation_summary_block` | C4/D3a/G73 | Once history outgrows the recent window, whole query-relevant source notes are selected within a fixed allowance and stamped with how far behind they run; final prompt budget may still evict the block. | `conversation_note_prompt_tokens` | `650` | YES (once history > window) |
...
| Chunk-level vector search | `src/retrieval/orchestrator.py:986` | C2 | Searches chunks of documents and all long turns directly; visibility (privacy, decay, archive, cluster, conversation) is enforced through the parent turn. Candidate maximum 3 chunks per parent; parents and excerpts compete until packing. | `retrieval_chunk_candidate_limit` | `30` | YES |
...
| Batch-summary leg (own) | `src/retrieval/orchestrator.py:2107` | — | This conversation's source-current batch notes, ranked by similarity; missing or changed source/output/policy manifests are withheld. | `retrieval_batch_summary_limit` | `3` | YES |
| Cross-conversation note leg | `src/retrieval/orchestrator.py::_batch_summary_lookup` | C4/G29/G53/G73 | v3 ranks independently indexed source notes. Whole-root freshness and every covered source's conversation/batch/cluster scope and exclusions still apply; private sources cannot cross scope. Own root is excluded. A note credits its own source batches; valid unindexed roots use the prior aggregate fallback. | `retrieval_conversation_summary_limit` | `2` | YES (cross half off under incognito) |
...
| Entity matching by vector | `src/retrieval/orchestrator.py::_match_entities_by_similarity` | A4/G41/G29 | Nearest visible graph entity by cosine, ranked by Postgres after the same source/project filter as exact and payload matching. Derived entities with externally populated vectors cannot become unscoped anchors or crowd out visible rows. | `codex_entity_match_threshold` | `0.85` | YES |
...
| Reranker bounds | `src/retrieval/reranker.py` | v3 repair | Caps candidates, model batch and full templated input length; oversized pairs warn and remain complete unscored fallbacks; fitting pairs still score. | `retrieval_rerank_candidates`, `retrieval_rerank_batch_size`, `retrieval_rerank_max_tokens` | `64`, `4`, `4096` | YES |
...
| Provenance collapse | `src/retrieval/orchestrator.py:2406` | C16 | Per-turn bounds apply to admitted fragments; selected complete raw turns exclude redundant excerpts, but summaries do not claim complete-source coverage. | `retrieval_collapse_enabled`, `retrieval_max_frags_per_turn` | `True`, `2` | YES |
...
| Complete episodic candidates | `src/retrieval/orchestrator.py:2861` | C2 | Complete eligible representations reach packing without word-prefix cuts. Documents with chunks use relevant excerpts; legacy documents without chunks remain complete candidates. | — | whole-block token budget | YES |
...
4. **RESOLVED v3 2026-09-24: `settings.ollama_num_ctx_mode` is read by the native foreground transport.**
   `fit` asks for the guarded prompt plus answer reserve; `max` asks for the capped serving window. A two-sided request-body control checks both and verifies that external compatible requests receive no Ollama `options`.
...
3. **RESOLVED for local foreground Ollama, v3 2026-09-24: `ollama_send_num_ctx = True` by default.**
   Native `/api/chat` now receives the requested window and emits a warning if it is disabled or clamped. Externally configured OpenAI-compatible providers have their own context policy; the explicit request is not a universal guarantee against a model's architectural ceiling. A real native completion and request-body controls checked the local path.

4. **RESOLVED v3 2026-09-23: vector anchor matching lacked project visibility.**
   The vector matcher now applies `*self._entity_source_filters()` inside its ranked SQL query, like exact/payload matching. Before repair, a disposable two-project control returned a hidden derived row for an unscoped query; after repair, it selects the visible conversation row and each project's own nearest row. Standard derived writers currently store NULL embeddings, so this qualified a latent/imported-vector path, not a measured leak from the working store.
...
| **Batch-summary output ceiling** | `src/workers/batch_summarizer.py:92` | — | Max generated tokens per original-source batch note. Was hardcoded 500, which truncated 2 of 3 summaries mid-sentence while `bg_model_output_truncated` logged it and nothing acted. | `batch_summary_max_tokens` | **1200** | YES |
...
| Feature | Where | Roadmap id | What it does | Setting | Default | On by default? |
|---|---|---|---|---|---|---|
| Batch summarisation of old turns | `src/workers/batch_summarizer.py::batch_summarize` | G11/G73 | Groups at least5 eligible warm/cold originals by token budget; original attributed notes require NLI support, otherwise complete source survives. Fingerprinted manifests detect edits/deletions; stale caches rebuild from either tier. | — | — | YES |
| Age-OR-decay selection | `src/workers/batch_summarizer.py::_eligible` | G11/G73 | A warm or cold source qualifies at original `decay_score < 0.3` OR age over the cutoff. Legacy cold unknown document status cannot enter a new compressed batch. | `settings.batch_summary_age_days` (and hardcoded 0.3) | 30 | YES |
| Coverage stamp (`batch_summary_id`) | `src/workers/batch_summarizer.py::batch_summarize` | G11/G73 | Stamps exactly the warm/cold originals a summary covers, in the same transaction. Stale-cache repair clears both tiers before regeneration. | — | — | YES |
...
| Independent conversation source notes | `src/workers/conversation_summary.py::run_conversation_summaries` | C4/G73 | v3 generates only from original attributed groups. New tails append independently; changed sources rebuild. Cached output never enters generation. | — | — | YES |
| Query-selectable conversation notes | `src/memory/conversation_notes.py`, `src/api/prompt_assembler.py::conversation_summary_block`, `src/retrieval/orchestrator.py::_batch_summary_lookup` | G73 | Derived part-vector index selects complete query-relevant source notes for the active prompt and cross-conversation retrieval, after source freshness/scope/privacy checks. Cross notes credit only their own source batches. Snapshot-valid unindexed roots fall back to the complete aggregate until background backfill. | `conversation_note_prompt_tokens`, `retrieval_conversation_summary_limit` | 650, 2 | YES (indexed rows) |
| Rolling-summary source snapshot | `src/memory/summary_snapshot.py` | C4/G73 | v3 policy3 binds original IDs/fingerprints, independent parts, output and verifier policy. Both readers reject stale/legacy/part-mismatched caches. | — | legacy rows regenerate | YES |
...
| Complete original-source groups | `src/workers/conversation_summary.py::_original_groups`, `::_default_llm` | C4/G73 | Complete attributed originals grouped before soft target; full request capacity checked. No generated input or source-prefix cut. | `conversation_summary_chunk_words`, `conversation_summary_max_words` (per-note target) | 3500 / 250 | YES |
...
| Source-supported independent note | `src/workers/conversation_summary.py::_source_note` | C4/G73/G75 | Current full-pair NLI required before generated note substitutes for source. Unknown/unsupported preserves full original; no mandatory-term retry. Long-source compression remains open. | `source_support_threshold`, `source_support_max_tokens` | 0.95 / 512 | YES |
...
| Feature | Where | Roadmap id | What it does | Setting | Default | On by default? |
|---|---|---|---|---|---|---|
| Explicit context pull (`context_for`) | `src/services/retrieval_svc.py:88` | E0 D4, C11, F1, G29 | Runs classify → memory-decision → orchestrate and returns structured fragments. With a known conversation identity it uses the classifier's prior-turn context and the same warm-turn pressure/count as chat; identity is separate from SQL visibility, so an `auto` pull can still search shared non-private memory. An unknown identity remains closed. Powers MCP `ice_context`, `/search`, and F1 preview. | `settings.context_budget_fallback` (when no `budget` passed) | `23_000` | YES |
| Explicit pull ALWAYS orchestrates | `src/services/retrieval_svc.py:130-138` | B2, spec rev 5 | The B2 retrieve/don't-retrieve decision is computed with the real conversation pressure when known and *reported* in the result but never used to answer empty-handed — `context_reliance` is forced to `Long_Term_Memory`. | — | — | YES |
...
### v3 batch-source notes (2026-09-21)
...
| v3 cold batch eligibility | `ColdStorage.is_document/decay_score`, `decay.py`, `batch_summarizer.py` preserve known document status and original decay for all-cold or mixed batch generation; NULL legacy status is not guessed eligible | nullable archive metadata, 5-source floor | YES for newly archived rows |
