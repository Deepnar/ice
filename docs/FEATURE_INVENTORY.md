...
| Rolling-summary source snapshot | `src/memory/summary_snapshot.py` | C4/G73 | v3 stores source IDs/fingerprints/timestamps plus output SHA256; SQL hashes avoid fetching full raw content for read validation. Missing cold/warm source or legacy NULL means unknown, never a fabricated backfill. | — | NULL for existing rows until regeneration | YES, both readers |
