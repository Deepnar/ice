...
| Cross-conversation summary leg | `src/retrieval/orchestrator.py:2148` | C4/D3b | Other conversations' composed original-source notes as overview hits (not globally reconciled narratives). Private conversations and public conversations containing private turns never leave their scope; the active conversation's own summary is excluded (the assembler injects it). | `retrieval_conversation_summary_limit` | `2` | YES (off under incognito) |
