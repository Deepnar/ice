...
| Evolving whole-conversation summary | `src/workers/conversation_summary.py::run_conversation_summaries` | C4/G73 | v3 compares output-bound source snapshots. Unchanged sources skip generation; newer tail can increment, edits/deletions/backfills rebuild from surviving representations. Both readers withhold stale/legacy snapshots. This is freshness, not semantic verification. | — | — | YES |
| Rolling-summary source snapshot | `src/memory/summary_snapshot.py` | C4/G73 | v3 policy version2 binds source/output identities, source support verdicts and current verifier/coverage policy. Prefix-era snapshots rebuild. Missing cold/warm source is unknown. | — | NULL for legacy rows until regeneration | YES, both readers |
...
| Bounded folding chunks | `src/workers/conversation_summary.py::_source_chunks`, `::_default_llm` | C4/G73 | v3 packs complete supported turn representations with recorded times before the soft group target. Full message token count, output reserve and margin must fit configured/observed capacity before submission. No per-turn prefix cap. | `conversation_summary_chunk_words`, `conversation_summary_max_words` (generation target) | 3500 / 250 | YES |
...
| Coverage-guided fold with retry | `src/workers/conversation_summary.py::_summarize_chunk` | C4/G29/G73 | v3 input representations share source support; generated fold output still uses term-coverage retry, which is NOT semantic verification. Complete output is retained without a word cut; recursive fold quality remains open. | `turn_summary_coverage_threshold` | 0.7 | YES |
