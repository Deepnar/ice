...
| Evolving conversation summary | `src/api/prompt_assembler.py:49`, `:306` | C4/D3a | Once the conversation outgrows the recent-turns window, its composed original-source notes are eligible, stamped with how many turns behind they run; final prompt budget may evict the block. | `conversation_summary_max_words` | `250` | YES (once history > window) |
...
| Cross-conversation summary leg | `src/retrieval/orchestrator.py::_batch_summary_lookup` | C4/G29/G53 | v3 independent source-note overviews. Every covered source must satisfy resolved conversation/batch/cluster scope and exclusions before ranking. Private turns/conversations cannot cross scope; own root excluded. Carries exact source batch IDs and conversation ID. | `retrieval_conversation_summary_limit` | `2` | YES (cross half off under incognito) |
...
| Independent conversation source notes | `src/workers/conversation_summary.py::run_conversation_summaries` | C4/G73 | v3 generates only from original attributed groups. New tails append independently; changed sources rebuild. Cached output never enters generation. Composed context can be large. | — | — | YES |
