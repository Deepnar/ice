...
| Feature | Where | Roadmap id | What it does (plain) | Setting | Default | On by default? |
|---|---|---|---|---|---|---|
| One-unit prompt ledger | `src/api/context_ledger.py:53`, used `src/api/main.py:687` | C16 | Counts every block of the prompt in real tokens and checks the total against the window the server actually allocated. | `token_count_safety_margin` | `1.20` | YES |
...
| Eviction plan | `src/api/context_ledger.py:107`, `src/api/main.py:698` | C16 | When the prompt does not fit, sheds blocks in a stated order and re-assembles once. Constraints, the user's question and the system prompt are never evicted. | — | order is code-fixed | YES (but see DEAD — only 2 of 6 blocks are actually reachable) |
| Extreme-pressure warning | `src/api/context_ledger.py:134`, `:152` | C16 | When everything sheddable is gone and it still does not fit, says so loudly rather than letting the server truncate from the front (where the memory lives). | — | — | YES |
...
| Today's-date anchor | `src/api/prompt_assembler.py:251` | T1 | Stamps today's date in the system prompt so dated memory fragments can be ordered against "now". | — | — | YES |
...
| Bookmark injection | `src/api/prompt_assembler.py:316`, `src/api/main.py:616` | C16/C1 | Up to 5 turns the user pinned are injected as standing context, word-capped at 500 each. (This parameter was declared and never read before C16.) | — | 5 turns / 500 words hardcoded | YES |
| Project session-start block | `src/api/prompt_assembler.py:301`, `src/api/main.py:637` | E4/D6 | A coding conversation opening a new sitting gets a "where was I" block: state, diffstat, constraints, tasks, decisions. Only on the sitting's first turn. | `session_gap_minutes` | `30` | YES (project-scoped conversations only) |
...
| Per-turn window cap | `src/api/prompt_assembler.py:193` | C16 | One turn may not eat more than a third of the recent window; an over-large turn degrades to its summary, then its abstract, rather than being cut mid-sentence. | `recent_window_max_turn_frac` | `0.35` | YES |
...
| Retrieval strengthening (write-on-read) | `src/retrieval/orchestrator.py:2665` | G38/Z1 | Retrieved turns gain access count and decay score, so useful memory self-promotes. Off makes retrieval reproducible run-to-run (40/40 identical vs 26/40 with it on). | `retrieval_strengthen_writes`, `decay_strengthen_amount` | `True`, `0.15` | YES |
| Codex edge reinforcement | `src/retrieval/orchestrator.py:1963` | A3 | Anchor edges surfaced by retrieval gain strength; a pending edge promotes to active once strong enough **and** its extraction confidence clears the bar. | `codex_reinforce_increment`, `codex_promote_strength`, `codex_promote_min_confidence`, `codex_strength_cap` | `0.15`, `2.0`, `0.5`, `10.0` | YES |
...
| Grounded query expansion | `src/retrieval/orchestrator.py:1650`, `:530` | A4 | Adds matched entities' canonical names and aliases to the BM25 search prompt, so lexical search finds turns using the full name. Nothing is generated — this replaced HyDE. **⚠ MEASURED 2026-08-20 AND IT CHANGES NOTHING:** ablating codex fragments while KEEPING expansion vs disabling the leg entirely (which kills expansion) scores **identically** — `episodic_lookup` 0.664/0.664, `temporal` 0.536/0.536 — and removing expansion **GAINS** 0.033 on `summary_synthesis`. 444 probes, arm B store, retrieval level. It fires, it is wired, it lands nowhere. | `codex_expansion_max_terms` | `8` | **YES — but inert by measurement** |
...
| Leg-diversity guarantee | `src/retrieval/orchestrator.py:2561` | A10 | Each leg's single best fragment is admitted first, so no leg is completely crowded out. Keys on `source_type` deliberately, not on `leg`. | `retrieval_leg_guarantee_enabled` | `True` — **but the setting has no reader; see DEAD** | YES (unconditionally) |
| Round-robin budget fairness | `src/retrieval/orchestrator.py:2592` | A10 | Each round, every leg contributes its next-best fragment, highest-scoring leg first — so episodic (dozens of fragments) cannot soak the whole remainder. Exhausted legs drop out and their share goes to the rest. | — | — | YES |
| Degrade-before-drop | `src/retrieval/orchestrator.py:2569` | C1/C3 | A fragment too big for the remaining budget is swapped for its trusted summary, then its one-line abstract, before being dropped entirely. | `turn_summary_coverage_threshold` | `0.7` (trust bar) | YES |
...
| Feature | Where | Roadmap id | What it does (plain) | Setting | Default | On by default? |
|---|---|---|---|---|---|---|
| Read-time raw-vs-summary choice | `src/retrieval/orchestrator.py:2770` | C1 | Both forms are stored; the query decides. Untrusted summaries are never used; exactness intents get raw; compression-tolerant intents get the summary. | `turn_summary_coverage_threshold` | `0.7` | YES |
| Keyword protection | `src/retrieval/orchestrator.py:2803` | C1 | If the matched keyword lives in the raw text but not the summary, raw is used and the fragment is marked non-degradable — degrading would delete the very term that made it relevant. | — | — | YES |
| Date stamping | `src/retrieval/orchestrator.py:2890` | T1 | Every episodic fragment (and every degraded form of it) carries `[YYYY-MM-DD]`, so the model can order events against the today-anchor. Dates only, never clock times. | — | — | YES |
| Fact dating | `src/retrieval/orchestrator.py:1481` | T1 | Codex facts render `(since YYYY-MM)` at month precision — day precision would imply false exactness. | — | — | YES |
...
1. **The eviction order is inverted in practice — 4 of 6 sheddable blocks can never be evicted.**
   `ContextLedger.DEFAULT_EVICT_ORDER` (`context_ledger.py:41`) names `session_start, slots, bookmarks, conversation_summary, recent_turns, evidence`, and `main.py:702`–`713` is written to honour all six. But `main.py` only ever calls `ledger.add()` four times (`:691`–`:696`): `system_prompt`, `recent_turns`, `evidence`, `user_message`. The four static blocks are appended *into* `messages[0]["content"]` by the assembler (`prompt_assembler.py:296`, `:302`, `:307`, `:317`) and therefore counted as `system_prompt` — which is in `NEVER_EVICT` (`context_ledger.py:50`). `evict_plan` skips any block with `size <= 0` (`:123`), so the plan it returns can only ever contain `recent_turns` and `evidence`. **Net effect: the documented rule "static before evidence" is exactly reversed — slots, bookmarks, the session-start block and the conversation summary are unevictable, and evidence is the first thing to go.** The `constraints` exemption in `NEVER_EVICT` is also moot: nothing anywhere calls `ledger.add("constraints", …)`.
...
| `lossless_flag` gate for codex extraction | `src/workers/post_flight.py:210` | C1 | A turn earns lossless treatment if it has code, is creative, is a document section, or is dense enough. Only lossless turns reach codex extraction. | `settings.turn_density_lossless_threshold` | 0.35 | YES |
...
| One-line abstract (hierarchy level 3) | `src/workers/post_flight.py:95` | C3 | Same LLM call emits a ≤20-word abstract used only for budget degradation. | — | — | YES |
| Document-section handling | `src/workers/post_flight.py:188`, `:210` | C12 | A document section forces `lossless=True` (its "assistant half" is absent, so entropy alone would starve the graph) and the prompt stops pretending it is a chat exchange. | — | — | YES |
...
| Extraction output budget | `src/workers/codex_extractor.py:770` | G32/a1 | Raised from 500 because truncation silently lost whole turns' extraction. | `settings.codex_extraction_max_tokens` | 1200 | YES |
...
| Relation canonicalisation threshold | `src/workers/codex_extractor.py:612` | G45 | Cosine above which an incoming relation is folded onto an existing one. Calibrated on 30 hand-built pairs. | `settings.codex_relation_canonical_threshold` | 0.82 | YES |
...
| **Codex-grounding NER tier** | `src/workers/codex_extractor.py:775` | **A9b** | Picks which tagger confirms the entities extraction is allowed to relate: `preflight` (micro-NER) or `background` (NuNER Zero). A9b moved clustering and key-term extraction to the background tier and left this call site on the micro-NER, on 10 turns of evidence; the setting exists so the two-arm re-seed can settle it at 293. **The default reproduces pre-2026-08-17 behaviour exactly.** ⚠ The tiers are not interchangeable here: the whitelist is a constraint, so the narrower tagger moves triplets from `codex_conf_grounded` to `codex_conf_rejected` rather than deleting them. | `codex_extraction_ner_tier` | **`preflight`** | YES (as `preflight` — `background` is OFF by default) |
...
| Move to cold storage | `src/workers/decay.py:163` | T3 D12 | Below 0.05 the row is copied to `cold_storage` (with conversation_id, is_private, batch_id AND its embedding) and deleted from `episodic_memory`. | `settings.decay_cold_threshold` | 0.05 | YES |
