...
**What this phase is:** Setting up PostgreSQL with the pgvector extension (which adds vector similarity search), and creating all the tables that ICE needs.

**What you'll learn:** Basic SQL, database migrations with Alembic, what pgvector is.

**Where to learn:**

- PostgreSQL basics: https://www.postgresqltutorial.com/ (read sections 1–5)
- What pgvector is: https://github.com/pgvector/pgvector#readme
- Alembic migrations: https://alembic.sqlalchemy.org/en/latest/tutorial.html

---

### Step 3.1 — Start the database with Docker

You already have Docker. Create the file `docker/docker-compose.yml`:
...
The image `pgvector/pgvector:pg16` is PostgreSQL 16 with pgvector pre-installed. You're also starting Redis here because you'll need it in Phase 6.
...
The `-d` means "detached" — they run in the background. To check they're running: `docker compose ps`. To see logs: `docker compose logs postgres`.
...
```
pip install sqlalchemy alembic psycopg2-binary python-dotenv pgvector
```
...
- `sqlalchemy` is the Python library for talking to databases without writing raw SQL
- `alembic` is the migration tool — it creates and tracks changes to your database schema
- `psycopg2-binary` is the PostgreSQL driver
- `python-dotenv` reads your `.env` file
- `pgvector` is the Python binding for pgvector
...
This creates configuration files. Open `alembic/env.py` and find the line that says `target_metadata = None`. You'll update this after creating your models, but for now leave it.

Open `alembic.ini` and find `sqlalchemy.url`. Change it to:
...
### Step 3.4 — Create the database models

Create `src/memory/models.py`. This file defines all the database tables as Python classes using SQLAlchemy. You do not write SQL — SQLAlchemy generates it.

You need to define these tables. For each, here is what columns to create (just copy these directly — SQLAlchemy syntax for each type is in their docs):

**conversations:**

- id: UUID primary key, auto-generated
- created_at: timestamp with timezone, default = now
- memory_scope_type: text (values will be "none", "auto", "project", "manual")
- cluster_ids: array of UUIDs (nullable)
- custom_filter: text (nullable)

**episodic_memory:**

- id: UUID primary key
- conversation_id: UUID, foreign key → conversations.id
- cluster_id: UUID, foreign key → context_clusters.id (nullable)
- parent_message_id: UUID, foreign key → episodic_memory.id (nullable, self-referencing — this is how branching will work later)
- batch_id: UUID
- timestamp: timestamptz
- topic_tags: array of text
- intent_tags: array of text
- context_reliance: text
- entropy_score: float (nullable)
- lossless_flag: boolean (nullable — NULL means not yet evaluated)
- raw_text: text (write-once)
- summary_text: text (nullable)
- embedding: Vector(384) — this is the pgvector type
...
- id: UUID primary key
- slot_name: text (values: "persona", "user_preferences", "tool_guidelines", "project_context", "guidance", "pending_items", "session_patterns")
- content: text
- token_count: integer
- version: integer, default 1
- last_updated: timestamptz
- updated_by: text (values: "user", "reflection_worker")
...
- id: UUID primary key (UUIDv5 derived from canonical name)
- canonical_name: text
...
- id: UUID primary key
- source_id: UUID, FK → codex_entities.id
- target_id: UUID, FK → codex_entities.id
- relation: text
- strength: float
- source_batch: UUID
- confidence: text (values: "pending", "active")
- valid_from: timestamptz
- valid_until: timestamptz (nullable — NULL means currently true)

**codex_events:**

- id: UUID primary key
- entity_id: UUID, FK → codex_entities.id
- event_type: text
- payload: JSONB
- timestamp: timestamptz
- batch_source: UUID
...
- is_active: boolean, default true
- source_batch_ids: array of UUIDs
...
- id: UUID primary key
- rule_id: UUID, FK → sentinel_rules.id
...
- conversation_id: UUID, FK → conversations.id
- session_date: date
- topics_covered: array of text
- decisions_made: text
- unresolved_items: text
- entities_updated: array of UUIDs
- patterns_observed: array of UUIDs
...
- id: UUID (original episodic turn id)
...
**curated_labels** (for the fine-tuning loop):

- id: UUID primary key
- batch_id: UUID
- prompt: text
- corrected_topic_labels: array of text
- corrected_intent_labels: array of text
- corrected_context_reliance: text
- created_at: timestamptz

**Where to learn SQLAlchemy models:** https://docs.sqlalchemy.org/en/20/orm/mapping_styles.html

---

### Step 3.5 — Create and run the migration
...
This generates a migration file in `alembic/versions/`. Open it and look at it — it describes the SQL that will be run.

Then apply it to create the tables:

```
alembic upgrade head
```

To verify the tables exist, you can connect to the database:

```
docker exec -it ice_postgres psql -U ice -d ice_db
```

Once in the psql shell, type `\dt` to list all tables. You should see all the tables you defined. Type `\q` to exit.

---

### Step 3.6 — Create the pgvector indexes

After the tables exist, you need to create indexes on the vector columns so similarity search is fast.

Create `scripts/create_indexes.py`. This script runs these SQL commands against the database:
...
`ivfflat` is an approximate nearest neighbor index for pgvector that makes similarity searches fast at scale. The `lists = 100` parameter is a tuning value — 100 is a reasonable default for datasets up to a few hundred thousand rows.

Run the script after the tables are created.

---

**Phase 3 is done.** You have a running PostgreSQL database with all tables created and indexed.
