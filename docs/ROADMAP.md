...
>
> **Correction (verified in code, 2026-07):** The micro-NER model *was* loaded and active during the experiments — [ner_utils.py](../src/retrieval/ner_utils.py) loads `models/ner/ner_model.pt` (present on disk) and only regex-falls-back if that file is missing. The paper's limitations section and earlier drafts of this roadmap wrongly stated NER silently fell back to regex; that is false. Codex's weak experimental numbers therefore come from the *other* handicaps (chunk size, corroboration trap, semantic-vs-lexical entity resolution, extractor strength), **not** from entity detection. The paper's "NER Fallback" limitation should be corrected too.
...
- [ ] **A2 NER-grounded extraction (hybrid CPU/GPU pipeline)** `(rework)` — CPU micro-NER (512-token chunks, 20–50 overlap) produces the confirmed entity list; the GPU LLM only maps relations *between those entities*; relations naming entities NER didn't confirm are rejected as hallucinations. This splits the cognitive load (extractor no longer hunts for entities, only reasons about connections) and should kill hallucinations like `fastapi uses fastapi`. NER is already loaded and used in pre-flight retrieval (see correction above) — this item reuses that same model in the *post-flight extraction* path; it is not a loading fix. Consider whether the NER model needs retraining to widen entity coverage for extraction's needs.
