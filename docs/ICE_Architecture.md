...
| 6 | [Hybrid Retrieval Orchestrator](#6-hybrid-retrieval-orchestrator) | the legs, RRF fusion, budgeting, scoping, time-scoped retrieval, leg-failure reporting |
...
A ConfigurableOrchestrator subclass exists for ablation studies; it does not redefine the classification logic but exposes flags that toggle retrieval legs on and off (bm25, vector, codex, procedural, cluster_restrict, dynamic_budget; the `rag` flag died with the leg, C12, and the `hyde` flag with the rewriter, G36). **The `hyde` flag never worked** — `_on("hyde")` had no caller and the subclass does not override retrieve(), so it toggled nothing for its entire existence (see §6.2).
...
**Population.** The FastAPI proxy inserts one row per user/assistant turn in store_turn_async (a BackgroundTasks callback that runs after the SSE stream closes). The Post-Flight Evaluator writes lossless_flag, summary_text, inject_raw, and is_document; the Batch Summariser writes separate source-bound batch notes for eligible old turns.

**Querying.** Two retrieval legs read this table (§6.1–6.2): the BM25 leg uses ts_rank over to_tsvector('english', coalesce(raw_text,'')||' '||coalesce(summary_text,'')) with an OR query built from full-query PostgreSQL-normalized lexemes (digits, Unicode and late terms retained), filtered by decay_score \> 0.2 AND is_archived = false, LIMIT 100; the vector leg uses the decay- **and recency-**weighted cosine score (1 - (embedding \<=\> :prompt_embedding)) * COALESCE(decay_score, 1.0) * (1 + 0.25·exp(−age_days/30)) under the same visibility invariant, LIMIT 100 — **C8 (2026-07):** the recency factor sits *inside* the SQL score (candidacy, not just post-fusion rank; a recent turn that misses the top-100 can't be rescued by bonuses), mirrors A11's codex formula, applies to the chunk leg and wide net too, and is skipped for Creative_&_Media (same recent-meta-is-noise rule as the bonuses). After fusion, retrieved turns are *strengthened*: access_count += 1 and decay_score = min(1.0, decay_score + 0.15) — **gated as a whole by `settings.retrieval_strengthen_writes` (default True; G38/Z1, 2026-08-13).** Zeroing `decay_strengthen_amount` stops the score moving but leaves the access_count UPDATE firing, and that write alone reorders equal-scoring candidates on the next query: two identical scoring runs agreed on **26/40** result sets with it on and **40/40** with it off, even though access_count has no reader in the retrieval path. Measurement harnesses must set it False; production leaves it True.**C2/C3 (2026-07): chunk-granular memory.** Chunking covers documents (C2) **and all long turns** (C3, > ~600 words — chapters, big technical answers): post-flight dispatches the chunker for both, and the 2h catch-up heals legacy rows (SQL length proxy). The shared chunker is **heading-aware** (C3: a markdown heading always starts a new chunk — section-level decomposition; no overlap is carried across a section boundary). In the vector leg, non-document parents stay in turn-level search and their chunks compete alongside until packing; only a selected complete source suppresses its redundant excerpts; document parents are excluded entirely. **Documents with chunks use excerpts.** A doc row found by text search injects only its keyword-relevant chunks (≤2, opening chunk as fallback) from the **episodic_chunks** table (chunked by workers/document_chunker.py using the shared chunker src/memory/chunking.py — the A1 sentence/code-aware packer, extracted to serve extraction windows, document chunks, and future C3/C12 alike). The vector leg excludes is_document rows entirely (a whole-doc embedding is semantic mush) and searches **chunk embeddings** instead (_vector_chunks: visibility — decay/archive/privacy/scope — enforced through the parent-turn join; ≤3 chunks per document; provenance points at the parent so strengthening lands on the turn). Legacy pre-C2 docs without chunks remain complete candidates subject to the final token budget until the runtime's 2h catch-up (run_pending_documents) heals them; new documents are chunked by a direct run_chunk_turn call inside the post-flight job (C7; private docs included — chunks inherit visibility via the join).

**v3 source boundaries (2026-09-13).** New chat/API and replay writes store
`source_spans`: ordered user/assistant character ranges over immutable raw text,
plus its SHA256. Document and explicit-note writers record document/user roles.
`memory/source.py` validates hash/ranges/roles; legacy or malformed metadata yields
unknown authorship, never an inference from embedded `Assistant:` strings.
The recent raw-message reader uses valid spans to preserve the actual pair.
Source spans and timestamp provenance survive cold archive/restoration. Additive
migration `c8f0d21a6e39` leaves old rows unannotated; it is not a role backfill.
Sentence-claim and semantic-verification consumers remain in the repair phase.

### **3.2 Codex (semantic) memory**

The Codex store is a versioned knowledge graph spread over four tables (full schema in §4): codex_entities (canonical name, aliases, tags, properties JSONB, auto-regenerated context_payload, embedding Vector(1024)), codex_edges (typed relations with strength, confidence ∈ \{pending, active\}, valid_from, valid_until, **learned_at, unlearned_at**), codex_events (append-only event log), and codex_snapshots (compaction output). It is populated by the Codex Extractor (§4.3) and the manual Codex Inject Watcher (§8.9), enriched by the Reflection worker, and queried by the Codex graph-traversal retrieval leg (§4.8).
...
The v3 `batch_summaries` table contains a nullable `source_manifest` alongside
its source conversation, legacy positional indices, rendered notes, embedding
and creation time. The worker selects non-private, non-document, non-lossless
turns with no coverage marker, older than the age setting or below decay0.3.
It groups by token budget (minimum5 turns), then composes independent notes from
complete original source groups. NLI-supported compression is allowed; otherwise
the complete attributed source is retained. `batch_summary_max_tokens` defaults
to1200 generated tokens per note. The complete provider request must fit; partial
responses never advance coverage. Embeddings cover every span of long output.

Manifest policy, source identities/fingerprints, note composition and output
hash must match on read. The worker unlinks and removes stale generated caches,
then rebuilds eligible originals. Source loading is bracketed by snapshots and
source rows are locked briefly for the final recheck and atomic coverage write.
Source deletion, privacy changes, output edits or legacy missing manifests cannot
silently leave an authoritative aggregate behind. Original turns are not deleted
by this worker. Retrieval is own-conversation scoped, top3 by cosine, and skipped
under non-current time scope. Source-current caches now survive archival using the shared warm/cold source
projection. Legacy missing metadata remains unverified; stale all-cold batch
caches are not regenerated by the warm-only batch worker.


## **4. Codex Knowledge Graph (v2)**

ICE v3 Codex retains graph navigation beside attributed source sentences. Relation categories remain extraction/type hints; no category establishes cardinality or authorizes expiry. Source-backed reconciliation decides whether assertions coexist or replace one another.

### **4.1 Entity and edge tables**

codex_entities (PK id — a deterministic UUIDv5 in CODEX_NAMESPACE for new entities) holds canonical_name (unique), aliases ARRAY(Text), tags ARRAY(Text), **entity_type Text (A7 — structural node type; ICE is general-purpose, so the vocabulary spans all domains: universal person/place/organization/event/concept/object, coding/research software/function/class/file/module/dataset, academic/business document/product, creative character/location/item/creature/faction; inferred from an entity's relations via _infer_entity_type, or set deterministically by the code graph E1b)**, **description Text (A7 — the enriched "note body", written by the reflection enrichment worker)**, properties JSONB, an auto-regenerated context_payload Text, embedding Vector(1024), and last_updated. **A7 rich notes:** _regenerate_context_payload assembles context_payload as a bidirectional Obsidian-style "note" — the description (note body), then Properties, then **Links** (outgoing edges, strongest-first) *and* **Backlinks** (incoming edges — what points *to* this entity, previously invisible) — so an entity's full connection picture reaches retrieval in both directions. codex_edges holds id, source_id/target_id FKs to entities, relation Text, strength Float (default 1.0 for all new extracted edges), source_batch UUID (the batch that asserted the edge), confidence Text ∈ \{pending, active\} (ordinary edge promotion requires a distinct source batch; reads never promote), **extraction_confidence Float ∈ [0,1] (A3)** — how much the extraction itself is trusted: seeded by NER grounding at write time (0.9 grounded, 0.7 no-NER chunk, 0.35 grounding-rejected; legacy edges 1.0), raised to the max seen on corroborating re-extraction — valid_from, and valid_until — **`NULL` means not retired, not verified true**; non-NULL means historically superseded — plus **learned_at / unlearned_at (bi-temporal, migration `505f12031434`, 2026-08-13)**. The two pairs answer different questions and conflating them is what made a wrong memory unauditable: `valid_*` is when the FACT was true in the world, `learned_*` is when ICE came to believe it. They diverge constantly — a fact true since 2019 learned yesterday, or a fact believed for months that was never true. Only the second pair can answer *"when did we start believing this?"*, which is the question asked after a fabrication is found. An invariant in models.py mirrors `valid_until` into `unlearned_at` on every retraction path (seven call sites across two modules), so a new retraction site cannot forget it. An edge's **effective trust** used by retrieval is extraction quality with bounded retention/recency ranking (strength carries usage dynamics; extraction_confidence carries extraction trust). Two auxiliary tables complete the graph: codex_events is an append-only event log (event_type ∈ \{edge_added, edge_expired, edge_strengthened, property_updated, context_appended\}, payload JSONB, batch_source, compacted Boolean), and codex_snapshots holds point-in-time full_state JSONB produced by the compaction worker.
...
- **`PROPERTY_RELATIONS`** (11 categories, written to the properties JSONB): identity (name, alias, nickname, full_name, title, description), demographics (age, gender, species, nationality, religion, birthday, blood_type), appearance, professional (role, occupation, profession, affiliation, status), contact_location, metadata_generic (type, genre, format, license, version, language), task_management, attribution (author), technical_specs, narrative_metadata, abstract_metadata. A new property observation expires the prior active edge and overwrites the JSONB key.
  ...
  **A property VALUE is also a node, deliberately (G33, 2026-08-08).** `handle_triplet` creates the object entity before it checks whether the relation is a property, so `kael --role→ fire mage` writes the property, an edge, **and** a `fire mage` entity. That node is kept rather than suppressed, because the edge behind it is what lets T4 build a timeline when the property *changes* — remove the node and the FK, the `edge_expired` event, and the property's history go with it. What was wrong was that its `context_payload` stayed empty forever: this branch was the only one of `handle_triplet`'s three that refreshed the subject alone (the negation and non-property branches always refreshed both ends), so the value node was a matchable pre-flight anchor that §6 then skipped for being empty — an anchor slot spent on nothing. Both ends are now refreshed and the value renders as `Backlinks: kael --role→`. ⚠ Regenerating exposed a second defect, also fixed: `_infer_entity_type` maps a relation to the type of the entity that has it **outgoing** (`role` ⇒ person, because a thing that *has* a role is a person), but voted over a flat list of both directions — so read from the far end it typed `fire mage` as a *person*. Incoming relations now vote only when the relation is **symmetric** (`married_to`, `colleague`, …); asymmetric incoming edges yield `entity`, which is unknown rather than wrong.
...
- **`SINGLE_VALUED_RELATIONS`** (11 categories): organisational_position (part_of, works_at, reports_to, managed_by, …), executive_role, succession, education, production_singular, personal_relationship (married_to, parent_of, child_of, sibling_of, …), biography_location (lives_in, born_in, died_in, …), deployment_ownership, logical_requirement, narrative_singular, conceptual_singular (instance_of, subtype_of). These are historical vocabulary hints; v3 no longer expires another value from this category alone.

### **4.3 Temporal versioning and automatic property updates**

handle_triplet is the heart of the write path. **G44 (2026-08-13): it first refuses triplets whose subject or object cannot name a node at all.** The measured store held entities literally named `8`, `3`, `6`, `2`, `d` — **typed `person`** — plus `i`, `the` and `~`; 155 of 3,671 entities were ≤3 characters. A node called `i` collects every speaker who ever said "I" and can never be looked up, so the edge hanging off it is not a weaker fact but an unreachable one. ⚑ The test is **functional, not a length rule**: `ai`, `ml`, `q4`, `eq` are two characters and perfectly good nodes, while pronouns, articles, bare numerals and punctuation refer to nothing whatever the writing style — which is what keeps this clear of the style-invariance rule G28 exists to enforce (a length rule is exactly the bet on convention that rule forbids). Rejection is logged at WARNING every time: the rate is the finding, and a silent skip would read as a clean graph.

Entity resolution (get_or_create_entity) is **three-stage** (G50, 2026-08-17): exact canonical_name match, then alias match, then a **normalisation-key match** — `properties.merge_key` (casefold · split digit/letter runs · collapse separator punctuation · **token order preserved**), matched against live entities only, with the arriving surface form recorded as an alias so the exact tiers catch it next time. Otherwise create with a deterministic UUIDv5 and a fresh embedding, stamping `merge_key` at birth. The third tier exists because the first two are exact-string and `canonical_name` is UNIQUE, so `gemma-4-e4b q4` and `gemma-4-e4b-q4` each minted their own row; relations have had `canonical_relation`'s three-tier ladder since G45 and entities had none, which was never a decision. It is an equality test, not a judgement: it never deletes a row, is not gated behind `codex_node_promotion`, and honours `protect_ids` exactly as promotion does. Migration `a1c4e7b90d22` backfilled 8,280 rows and added a partial index; 51 groups on the live store collapse (`20gb`/`20 gb`, `ai ml`/`ai/ml`). ⚠ Token order is preserved deliberately — sorting finds more groups and two of them are converses (`shiva without brahma`/`brahma without shiva`, live at cosine 0.9639). **Node promotion (G44's second half, `codex_node_promotion`, default OFF):** when a strictly more specific name arrives for a generic node already stored — `master plan` against a bare `plan` — the specific node becomes canonical and the generic name resolves to it thereafter, so later mentions land on the node worth traversing instead of sprouting a second one. ⚠ **Only zero-edge stubs are eligible.** Promotion merges two identities, and `plan` is not always `master plan`; doing it to a node that already carries facts would silently re-attribute them, whereas a stub has none to re-attribute. It is off by default for that reason — the aggressive form (promoting nodes with edges, migrating those edges) changes stored history and is a separate decision. All extracted assertion types now share one writer (v3, 2026-09-19):

- Exact directed relation and polarity observations reinforce once per source
  batch; repeated reads or the same batch never promote confidence.
- A new edge starts pending at strength1. Property and negative assertions have
  no privileged expiry or confidence path.
- Every same-relation target/polarity alternative and known opposition is a
  candidate comparison. Complete current linked source units must share a
  conversation and authoritative role, with original source timestamps strictly
  ordered, before automatic reconciliation. Unknown authority retains both.
- Reconciliation compares complete passages and speaker/time metadata. A rejected
  replacement rolls back prior expiries from its comparison set. Property JSON
  projects all live positive values, not last-write-wins state.
- Maintenance shares the evidence boundary and prompt; application rechecks it.
  Manual reconciliation requires an explicit keep-edge subset. Expiry journals
  the decision and refreshes both endpoint notes and property projections.

Current `valid_*` expiry remains a recorded system decision, not an inferred
real-world event date. Older imports cannot overwrite newer source assertions;
source times remain separately labeled. Autonomous event-time interpretation,
cross-conversation author identity, semantically different open-vocabulary
relation candidates and long-source verification remain unqualified.

### **4.4 Extraction pipeline**

> **⚑ EXTRACTION HAS ITS OWN MODEL SETTING (G63, 2026-08-27).** `codex_extraction_model` resolves as `model_override or settings.codex_extraction_model or get_bg_model_name()`, defaulting to `hf.co/numind/NuExtract3-GGUF:Q8_0`. It exists because **one background pin served ten jobs** — turn summary, batch summaries, conversation summary, cluster naming, reflection, the maintenance agent, procedural extraction, document-kind, the raw slicer, and extraction — and only the last wants a specialist. Pointing the shared setting at NuExtract3 would hand a JSON-template filler to the summariser and the cluster namer, which fails *silently*: the summaries keep arriving, just wrong.
> ⚠ **NOT the conflict reconciler.** `make_llm_reconciler` lives in the same file and is a one-word **reasoning** call (`expire_old` / `keep_both` / `reject_new`); it deliberately follows the general background model. It carries a docstring saying so, because it is the natural thing to sweep along when "the extractor" is repointed.
...
>
> **The background extractor is now `NuExtract3-Q8_0`, not `qwen3:4b-instruct`.** Measured over 60 random turns with the maintainer's own blind labels: **63% of stored triplets true vs 15%, and 0 reversals in 20 judged facts against 6.** That is the only intervention in this project's history to move graph correctness. See [PROVENANCE](PROVENANCE.md) 2026-08-24 and [ROADMAP G63](ROADMAP.md#g63).
>
...
8. **Write + reconcile (A6, v3 repair 2026-09-13).** Canonicalization's relation-separation map prevents inverse/polarity merges; it does not authorize expiry. The narrower opposition candidate map and legacy supersession cues nominate pairs for a source-aware reconciler. Every candidate uses that reconciler or review; none expires deterministically from an opposition label. The model consumes full supplied source within `codex_reconcile_input_tokens` (default8192); over-bound input, missing source, incomplete response or anything other than an exact decision retains both claims for review. The ordinary writer reinforces only the same direction, relation and polarity; sharing endpoints with a different relation does not authorize expiry. As of 2026-09-19, property/single-valued/negative assertions use the shared two-source boundary described in §4.3. Background polarity/opposition pairs become deduplicated `codex_contradiction` proposals, never automatic processing-time expiry. No claim of complete conflict semantics from these controls.
...
> **⚑ SUPERSEDED 2026-08-13 — THE VOCABULARY IS NOW OPEN (G45). Read this box
> before the three subsections below, which describe the closed-list era.**
>
> The closed list was the defect, not the filter around it. Re-measured on the
> 293-turn store: **1,259 relations destroyed in a single seed**, including true
> facts — `i --didnt_get--> csi`, and `my father --t_get--> first` where an
> apostrophe split `didn't get` into `t get`. Neither branch of the old choice
> was acceptable: dropping kept 32% of what the model produced, forcing the enum
> kept 100% and got ~78% of them wrong.
>
> **`canonical_relation` replaces the gate**, in three tiers: an exact
> vocabulary hit (unchanged, keeps existing relations stable); else
> canonicalisation against relations **the graph already holds** — embed,
> compare, reuse above `codex_relation_canonical_threshold`; else **accept it as
> a new relation**. `ALLOWED_RELATIONS` survives as tier 1 and as a kill-switch
> (`codex_relation_open_vocabulary=False`). `_relation_gloss_cache` now reads
> the open vocabulary, so G34's relation fit does not go inert on relations
...
> 0.86 threshold then in force. Merging `before` into `after` reverses every
> temporal fact in the graph, silently. `_ANTONYM_PAIRS` (converses) and a
> passive-marker check run first; the threshold only chooses between pairs
> already known safe. ⚠ **The live threshold is `0.90`, not the 0.82 this line claimed** (`config.py:521`, corrected 2026-08-23) — and the guard it describes was structurally unable to fire on `is_*`/`is_*` pairs until G45's polarity and argument-role checks were added. Negations stay
> out — A8's `negated` flag owns those, precisely so the vocabulary does not
> double. See TRAPS #26 and [specs/G45_open_relation_vocabulary.md](specs/G45_open_relation_vocabulary.md).
...
### **4.5b Cluster naming and description — ⛔ UNMEASURED, and one attempt to measure it was invalid**

`_create_cluster_from_members` generates BOTH a `name` and a `description` from the same members, each grounded with `_recurring_member_entities(turns)`. **Neither has ever been judged on real clusters.**

⛔ **A 2026-08-27 attempt produced a 62-76% "wrong" rate and is RETRACTED**: the harness fed the namer *five arbitrary consecutive turns* with `recurring_entities=None`, so it asked for the shared subject of turns never established to share one, without the grounding production always supplies. It measured the harness. ⇒ **G74**, and the open question underneath it is whether the defect is the naming prompt or the CLUSTERING — if members are weakly grouped, there may be no right name available. Judge cluster coherence first, then naming conditional on it.
...
10. **BM25 (full-text)** — _bm25_episodic. Postgres ts_rank over to_tsvector('english', coalesce(raw_text,'')||' '||coalesce(summary_text,'')) against an OR query of all full-query lexemes from the same PostgreSQL `english` normalization; lexemes are quoted before casting, so punctuation cannot become query syntax. The historical BM25 name denotes `ts_rank`, not BM25 scoring. Hard filters decay_score \> 0.2 AND is_archived = false *under current mode* (under a TimeScope window a timestamp predicate applies and both filters relax — §6.11), optional conversation_id and cluster_ids scope. LIMIT 100, ordered by ts_rank DESC.
...
**Retrieval calls no model at all.** A `_hyde_rewrite` method used to sit in the orchestrator, asking the background model to restate the prompt as a dense search query before the lexical leg ran (query *reformulation*, never actual HyDE — it did not fabricate a hypothetical answer document). The post-paper review (roadmap P0.1) rejected shipping the real thing: it would invent specifics about the user's *private* history with a small model, and hallucinated specifics corrupt the noise-sensitive BM25 leg. Production uses **grounded query expansion (A4)** instead: the BM25 search prompt is expanded with the canonical names + aliases of the entities the codex leg actually matched (§4.8) — expansion terms come from the graph, not a generator.

**⚠ The method was DELETED in G36 (2026-08-09), and the reason matters beyond the deletion.** It was unreachable by every path, and had been since before `v2-paper-eval`: the call site was commented out, nothing set `_force_hyde`, and the ConfigurableOrchestrator's `hyde` flag was never read by anything (`_on("hyde")` had no caller; the subclass does not override `retrieve()`). **So Experiment 1's `full_ice_no_hyde` arm was the same configuration as `full_ice`** — its runner disabled HyDE by stubbing a method nothing invoked — and the reported "HyDE has minimal impact (possibly because the background model is weak)" has the wrong cause: it never ran. This is the same shadow-subclass drift G19 found in `recency_boost`, which retro-tainted two Experiment 3 arms the same way. `self.bg_client` went with it (one OpenAI client built per retrieving request, for this single consumer). The paper's Experiment 1 table is corrected at FINAL, not retro-edited; `experiments/` stays frozen.
...
The model loads from local cache only, serializes access, leaves pairs beyond
4096 template-inclusive tokens unscored with a warning, retaining complete fallback evidence while scoring fitting pairs, computes only
...
- **Clamp** bonus ∈ [`settings.retrieval_min_total_bonus`, `settings.retrieval_max_total_bonus_multiplier`] = [-0.9, 4.0] (effective multiplier range [0.1×, 5.0×]).

17. **Pre-RRF adjustments** in _rows_to_fragments (episodic rows only, applied before RRF so they influence per-leg ranking): bookmark ×1.5, complete representations without word-prefix caps, and the **T1 date stamp** — every fragment (and its degrade/abstract forms) is prefixed [YYYY-MM-DD] from the turn's timestamp. *(The time/turn-recency additive tiebreakers previously described here were dormant dead code — no leg SELECT carried `timestamp` since pre-experiment commit 616d770 — and were removed by T1 rather than silently awakened when the column returned for date-stamping.)*

18. **Sort** by score descending.

19. **`_session_diversify`** (max_per_conversation = 3): applies only to candidates that fit during final packing; the active conversation is uncapped; foreign conversations are capped at 3 each; fragments with no conversation_id (Codex, procedural) are always kept.
...
**⚠ Silent truncation was real and measured before G32(b).** An oversized prompt to `tinyllama` logged `predicted=2909 / actual=2047` — the server cut the prompt to the model's window. Truncation keeps the **newest** messages, so retrieved memory can be destroyed first. **v3 G32(b), 2026-09-24:** local foreground generation now posts to native `/api/chat`, and `ollama_send_num_ctx` defaults on. `fit` asks for the guarded prompt plus generation reserve; `max` asks for the capped known serving window. Native NDJSON is translated to client SSE; thinking remains separate from answer content; only a completed nonempty generation enters post-flight memory. External configured models retain OpenAI-compatible SSE without Ollama-native options. The earlier `/v1` shim discarded `options`, making the previous setting inert. This repair does not make the model's architectural ceiling larger: the measured 0.30.7 behavior truncated `tinyllama` at 2,048 even natively, with HTTP 200 and `done_reason: "stop"`. Reconcile predicted-vs-actual token counts and treat an over-ceiling model as a serving limitation, not proof that every native reply contained the whole prompt.
...
**v3 repair, 2026-09-19.** The earlier ledger description overstated its
consumer: static blocks were inside the protected system message, so only recent
turns and evidence could actually be evicted. `assemble_prompt` now reports
structured incremental costs, including envelopes and the evidence acknowledgement.
`assemble_budgeted_prompt` reassembles and remeasures after optional-block removal;
all six optional blocks are reachable, with static blocks first. Required project
constraints are fetched separately on every request, including within a sitting.
Known required overflow returns HTTP400 before generation/graph usage; unknown
capacity warns that fit is unmeasured. Generation reserve is not silently halved.
Counts and graph usage describe final survivors. These remain ICE tokenizer counts
with a margin, not guaranteed exact counts for every future cloud tokenizer.
Bookmarks use complete source-supported turn representations and source timestamps,
never unchecked summaries or500-word prefixes; whole-block budget eviction applies.
...
**G29 (v3, 2026-09-23):** `memory/recent_window.py` is the single settings-backed base ladder and overhead estimate. B2 uses it for the pre-retrieval memory-pressure prior and the C4 summary job uses it for its model-agnostic row-creation gate. Retrieval starts from that same base but still applies turn-density and classifier-label adjustments before assigning its actual recent budget. A settings sweep now reaches each reader; the base estimate is deliberately not a claim that it always equals the request-adjusted retrieval allocation.
...
**⚠ A scope that resolves to NOTHING means nothing, not everything (G37, 2026-08-09).** `_codex_scope_sets` used to end with `if pid is None and not batch_ids: return None, None` — UNSCOPED. A conversation has no stored turns *before its first turn*, so the codex leg served the whole store on turn one of every manually-scoped conversation; exclusions reached the same branch by emptying a closed set. The deny sets held throughout (forbidden material never came back), but the closed set stopped being closed. This is the rule C6 wrote for `_conv_scope_filter` — "an empty set must match nothing" — arriving at the codex and procedural legs, and it is the same fix the `pid` test above applied to project attachments long ago. **The discriminator is key PRESENCE, not truthiness:** a scope that names no conversations at all (cluster-only, timescope-only, exclusion-only, `auto`) restricts nothing and stays unscoped, which is what keeps ordinary traffic working — `_apply_document_visibility` only *extends* an existing `conversation_ids` and never introduces the key under `auto`.

**⚠ A resolver that FAILS must not widen (G36, 2026-08-09).** Both codex-side resolvers used to fail open. `_codex_scope_sets` returned `(None, None)` on any exception — and `None` there means UNSCOPED, not "no scope", so failing to resolve the scope the caller asked for handed the leg the whole graph. `_resolve_exclusion_sets` was the mirror: it cleared the deny sets, and the graph then served precisely the conversations the user had excluded. Neither said a word. Both now **fail closed** — empty sets, the same value the `isolated` branch returns, so every downstream admission point handles it unchanged — and both emit `retrieval_leg_failed`. Neither is reachable without a scope (the no-scope and no-exclusions cases return earlier), so `auto` traffic is untouched. Pinned two-sided in `tests/test_retrieval_failopen.py`.
...
Every retrieval leg and post‑processing step can be independently enabled or disabled through a ConfigurableOrchestrator (a subclass of HybridRetrievalOrchestrator). An overrides dictionary—keyed by leg name ("bm25", "vector", "codex", "procedural", "batch_summary", etc. — "rag" was removed with the leg, C12) and post‑processing step ("rrf", "cluster_restrict", "session_diversify", "dynamic_budget", "keyword_boost", "recency_boost", "timescope")—controls whether each component participates in retrieval. The `timescope` flag (T2/T3) sets `timescope_allowed = False`, forcing every request to CURRENT mode — all temporal branches collapse to pre-Track-T behavior (the FINAL experiments' temporal ablation seam). Setting a key to False causes the corresponding leg to return an empty list or the corresponding transform to be skipped. This mechanism is used by the ablation experiments reported in the evaluation section, where features are added cumulatively from a bare vector‑only baseline to the full ICE stack, measuring the incremental contribution of each architectural addition.

**⚠ Two of these flags have been found to toggle nothing, and both corrupted a published ablation arm.** `recency_boost` rebound its own module's copy of the constants while the parent scored from the parent's (G19, 2026-08-08 — Experiment 3's `add_keyword_boost` and `full_ice` were therefore the same configuration). `hyde` was never read at all: `_on("hyde")` had no caller and this class does not override `retrieve()`, so Experiment 1's `full_ice_no_hyde` was the same configuration as `full_ice` (G36, 2026-08-09 — the flag and the method are now deleted). **The shape is the subclass mirroring parent internals, and only a run can see it — an import check cannot.** `tests/smoke/test_ablation_flags.py` pins the two known instances; G19 owns folding the mirroring away entirely, which is the real fix.
...
> **⚑ ICE RELEASES ITS OWN BACKGROUND MODEL (2026-08-27).** When the queue is genuinely empty — nothing overdue, running or queued — the runtime calls `bg_client_factory.release_bg_model()`, a native `POST /api/generate {"keep_alive": 0}`, guarded by a flag so it fires once per idle stretch and is cleared the moment any job starts. **Why it must be explicit:** ICE posts background work to Ollama's OpenAI-compatible endpoint, which **silently drops `keep_alive`**, so without this ICE inherits the host's `OLLAMA_KEEP_ALIVE` — measured 2026-08-27 on the dev machine (`-1`, deliberate), three background models sat resident at ~14 GB of a 24 GB card for jobs finished hours earlier. ⚠ This is the **small half** of G32(a); per-*request* `keep_alive` still needs the native chat endpoint. Settings: `bg_release_after_drain`, `bg_release_timeout_seconds`.
...
- **Key-term extraction** (extract_key_terms) — the turn's MUST-PRESERVE vocabulary: named entities via **NuNER Zero** (the *background* NER tier — the code has passed `tier="background"` since A9b; this doc and the function's own docstring both said MicroNER until 2026-08-27, verified at runtime as `background_ner_loaded model=numind/NuNER_Zero`), reusing the worker's already-loaded embedder — no extra model copy; figures (numbers/dates/units), identifiers (snake_case, CamelCase, dotted paths, acronyms).

> **⚑ THE MUST-PRESERVE INSTRUCTION WAS MANUFACTURING FABRICATION (2026-08-27).** The prompt used to order *"every one of these must appear verbatim in your summary"*, with a harder retry (*"include each of them verbatim this time"*) that fired on **38 of 70** turns. A model that cannot ground a term is being told to include it anyway, so it invents context to carry it — and `summary_coverage`, a presence metric, then scores it well for having done so. Measured on `gemma4:e4b`, n=70, one changed substring: fabrication **15.7% → 2.9%**, faithful **82.9% → 95.7%**, incomplete unchanged. Both strings now ask for the terms the turn *supports* and explicitly permit omission.
...
Three independent schedulers (§3.7), all every 5 400 s, max_retries=2, default_retry_delay=60: decay.apply_decay (episodic, access-weighted with creative floor, archive at 0.1, cold-storage move at 0.05), codex_decay.decay_codex_edges (retention decay to a 0.1 floor, without confidence demotion or temporal expiry — **conversation edges only; derived static-analysis/fact edges are decay-exempt** — the code graph is regenerated from source, not earned through use; §12.2), procedural_decay.decay_procedural_patterns (boolean deactivation after 180 days if \< 3 reinforcements).
...
batch_summarizer.batch_summarize (§3.8, runtime-scheduled every 2 h + the session-end burst) groups eligible not-yet-summarised turns by token budget and writes source-bound batch note rows. Distinct from §8.13's conversation summaries: batch rows are frozen *ranges* of old turns; the conversation summary is one *evolving* whole-conversation object.
...
**v3 source freshness (2026-09-20).** Each row also carries nullable
`source_manifest`: an output-bound snapshot of source IDs, SQL fingerprints and
recorded timestamps. Maintenance compares snapshots rather than only the newest
timestamp. Unchanged sources skip generation; strictly newer additions may extend
a valid checkpoint. Edits, deletions, older/same-time imports and legacy NULL
snapshots rebuild from surviving representations without reusing the invalid old
...
- retrieval – reports the active retrieval legs and the total tokens injected. (It also carried `hyde_used`, permanently False, until G36 deleted the rewriter.)
...
**Tuning constants are settings, not code (G9, 2026-08-08).** This paragraph used to say the opposite — that worker and retrieval constants were module-level Python, not environment-configurable — and that is no longer true of any of them. Roughly **160 values** moved: the retrieval scoring family (bonuses, penalties, word brackets, recency bands, RRF k, per-leg candidate limits, the codex trust floors and traversal depth), the **intent-dependent leg-weight table**, the dynamic-budget ladders, the decay/promotion/archival dynamics, the write-path gates, clustering, the maintenance agent's per-run caps, and a Tier-2 operational band (timeouts, batch sizes, cost estimates). What deliberately stayed in code: identifiers, controlled vocabularies, prompts, paths, and training hyperparameters — those are schema or content, not tuning.

Three properties of the move matter more than the list:

- **Values are read at the point of USE, never bound at import.** Six were defaults in a function signature (`_apply_rrf`'s `k`, `_session_diversify`'s cap, `_vector_chunks`' recency boost, `chunk_text`'s size and overlap, the classifier's context turns and word budget) — evaluated once at import, so overriding them at runtime changed nothing. They resolve in the body now. This is what makes Z1's `--knob` sweep possible at all, and it is why nothing may cache a setting on a module global or an instance attribute.
- **Decay is stored as a DAILY target, not a per-cycle rate.** `decay_daily_unaccessed = 0.95`, with the per-cycle multiplier derived as `daily ** (1/cycles_per_day)` — 0.9968 is not a number anyone can tune. **Cycles-per-day is itself derived** (2026-08-09) from each decay job's own entry in `maintenance_intervals`, and is deliberately *not* a setting: the job applies its multiplier once per run, so the count must equal `86400 / cadence` or the stated daily target is not the decay that actually happens. Change the cadence and the daily target is preserved automatically; `tests/test_dynamics_invariants.py` pins that at six cadences, and an unresolvable cadence warns rather than guessing in silence.
- **The leg-weight table is a validated nested dict.** `retrieval_leg_base_weights`, `retrieval_leg_profiles` (intent → per-leg override) and `retrieval_leg_topic_overrides`, resolved by `src/retrieval/leg_weights.py`, which validates **on every read** — an import-time check would only ever see the defaults, and the sweeper writes at runtime. Unknown leg names raise (closed set, always a bug); unknown intent/topic labels warn once and are ignored, because the live label schema changes with the checkpoint and a v1 rollback must not take retrieval down. It found a real dead row on its first run: the profile keyed on `Null_Noise`, which is a **topic** label and can never appear in `intent_tags`.

`tests/test_settings_freeze.py` pins every moved value against the git blob at the pre-G9 commit rather than against a retyped list. **It is meant to be retired knob-by-knob as Z1 tunes, not weakened.** The B2 memory-decision weights (ltm_decision_threshold, ltm_prior_bias, ltm_length_weight, ltm_pressure_midpoint_tokens, ltm_pressure_scale_tokens, ltm_bump_creative/reference/referential/low_confidence) **are** in settings, added with that rework. The former classifier-path mismatches are **resolved (2026-07, G1)**: both fine_tune.py and drop_zone.py now read settings.classifier_model_path, and fine_tune.py promotes a validated candidate onto that path — so the weekly fine-tune's input, output, and the active inference path all agree.
...
### v3 retention/support separation — 2026-09-13
...
Complete raw turns, supported summaries and existing long-turn chunks remain
alternatives until packing. Oversized rejected candidates consume neither the
per-turn nor foreign-conversation allowance. Both relevance packing and fallback
round-robin continue past non-fitting candidates. Only complete raw source text
carries full-source coverage; degradation clears that marker. Cold retrieval uses
the shared representation selector and no300-word cut; legacy archives lacking
...
`SUMMARY_SOURCES_SQL` presents warm and cold evidence once per source ID, giving
live rows precedence over cold duplicates. Snapshot fingerprints use unchanged
source fields across tiers. Cold batch coverage is stored with ON DELETE SET NULL
and restored only for an existing summary. Own/cross-summary scope, privacy and
source credit use the same projection before ranking. Cluster membership uses
the shared inclusion/exclusion builders; unknown legacy membership remains
