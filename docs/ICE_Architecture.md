...
19. **\`\_session\_diversify\`** (max\_per\_conversation = 3): iterates sorted fragments; the active conversation is uncapped; foreign conversations are capped at 3 each; fragments with no conversation\_id (Codex, procedural) are always kept.
...
| **PUT/GET** | /user-control/conversations/\{conv\_id\}/scope | set/read memory\_scope\_type (none\|auto\|project\|manual — validated since C6), cluster\_ids, the project attachment, and C6's three id sets (included\_conversation\_ids, excluded\_conversation\_ids, excluded\_cluster\_ids; each None = unchanged, \[\] = clear) |
...
| **POST/GET/PUT/DELETE** | /user-control/documents, /documents/\{id\}, /documents/\{id\}/enabled | C12 documents (§14): POST {path\|blob, conversation\_id, kind, dry\_run} ingests one (dry\_run returns the section count + estimate); GET lists the library with `enabled_here` per conversation; PUT flips the live enable toggle; DELETE routes through C10's cascade |
...
- context\_ready – reports the final fragment count and a breakdown by source (codex, episodic, procedural).
