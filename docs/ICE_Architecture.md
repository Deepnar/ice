...
### **6.11 Time-scoped retrieval (Track T: T1–T3, 2026-07)**

Storage always recorded time on three layers (episodic `timestamp`, bi-temporal codex edges, the event journal), but retrieval read none of them — every query answered "now." Track T makes the stores answerable *at* a time. Spec: docs/specs/T_temporal.md; T4 (the timeline builder / evolution surfacing, `src/retrieval/evolution.py`) is designed there but **not yet built**.
