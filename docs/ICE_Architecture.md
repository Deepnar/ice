...
After commit, it dispatches extract\_codex.delay(...) (only if lossless), extract\_procedural.delay(...) (always), and — for is\_document turns — chunk\_document.delay(...) (C2; runs for private docs too, since chunk visibility is enforced through the parent join). The codex/procedural dispatches are skipped entirely for private turns (§6.10). If the Celery enqueue itself fails (broker down), store\_turn\_async appends a JSONL entry to data/post\_flight\_buffer.jsonl as a write-ahead log for later replay.
