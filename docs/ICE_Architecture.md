...
The Codex retrieval leg (\_codex\_graph): (i) extracts prompt entities via the shared NER; (ii) resolves them by vector fuzzy matching; (iii) optionally restricts to a conversation scope (entities whose codex\_events.batch\_source appears in the conversation's batch set); (iv) **strength-gated BFS traversal (roadmap A3)** to max\_depth = 3 over edges where valid\_until IS NULL: the matched entity's *direct* edges always expand, but deeper hops only follow edges with strength ≥ CODEX\_DEEP\_STRENGTH\_FLOOR (1.0), so weak/decayed edges no longer pull the whole 3-hop neighbourhood into context; appends "\[Entity: \{canonical\_name\}\]\\n\{context\_payload\}" per visited entity; (v) returns a single ContextFragment of type codex whose score is **graded 1.0–1.5** from the mean strength of the matched entities' active edges (replacing the old binary 1.5× active-edge boost). **Retrieval-reinforcement (A3):** `_reinforce_codex_edges` bumps the matched-entity *anchor* edges' strength by CODEX\_REINFORCE\_INCREMENT (0.15, capped at 10.0) each time they're surfaced — the Codex analog of episodic access/decay strengthening (§6.5), balanced by codex\_decay. If the NER extracts no entities, MERA is invoked as a fallback (disabled by default; gated by the mera ablation flag). *Note: pending edges are traversed here — retrieval never filtered on confidence='active' (only the old score boost did).* Full per-edge (rather than per-blob) scoring awaits the multi-fragment representation (roadmap A10).
...
12. **Codex graph traversal** — \_codex\_graph. NER → vector fuzzy matching (threshold=0.85) → strength-gated depth-3 BFS over valid\_until IS NULL edges (deep hops require strength ≥ 1.0), collecting context\_payload per visited entity, with retrieval-reinforcement of anchor edges (§4.8, A3). MERA fallback when NER is empty (disabled by default). Score graded 1.0–1.5 from matched-edge strength.
