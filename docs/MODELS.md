...
|---|---|---|---|
| **background / codex extraction** | `qwen3:4b-instruct` | Ollama | every measurement was taken on this. ⚠ `.env` still pins `BACKGROUND_MODEL_NAME=gemma4:26b-a4b-it-q4_K_M` — **stale leftover config to be changed before anything ships, NOT a live production setting**: nothing is in production, the proxy is not serving, and no user has been handed a 26B-built store. ⚑ **Standing decision (maintainer, 2026-08-22): ICE will NEVER use a model that large for background work** — background runs on a laptop card beside the user's chat model, so the candidate set is small models only. See [G58](ROADMAP.md#g58). |
