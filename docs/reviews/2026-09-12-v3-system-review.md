...
| Explicit context pulls have their own preparation and serialization. | `src/services/retrieval_svc.py::context_for` | It classifies without conversation context, budgets with zero turn/history counts, and omits fragment origin sets/producing legs in its returned dictionaries. Forced retrieval is intentional; loss of provenance and accidental preparation drift should not be. |
| Manual cluster assignment writes the legacy single cluster column. | `src/services/clusters.py::assign_turns`; FEATURE_INVENTORY “DEAD OR INERT” | Verify round-trip visibility through link-based readers before describing manual control as complete. No live leak or failure is claimed here. |
