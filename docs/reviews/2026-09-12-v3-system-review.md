...
| Explicit context pulls had their own preparation and serialization. **Repaired 2026-09-23:** caller identity is separated from SQL scope, classifier/history pressure use that conversation, and `/search` passes it through. | `src/services/retrieval_svc.py::context_for`; `src/memory/conversation_stats.py` | Before repair, `ice_context(conversation_id=A)` made an `auto` search closed to A, unlike chat. Disposable controls now find a turn in a second non-private conversation, preserve unknown-ID closure and incognito isolation. Returned dictionaries already include origin batch/edge IDs and producing leg. Forced retrieval remains intentional. |
| Project constraint lookup scanned all projects without a project identity. **Repaired 2026-09-23:** a bare projectless pull returns no project decisions, while `ice_context(project=…)` chooses a complete closed project scope. | `src/services/retrieval_svc.py::constraints_for_task`; `src/services/scoping.py::resolve_project_pull_scope`; coding-core/MCP controls | Two projects can both have a `config.py` constraint. Only the selected project's rule and eligible memories should surface; a basename match alone cannot choose whose decision applies. Project-attached conversations keep the E8 payoff. |
