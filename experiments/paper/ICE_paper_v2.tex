...
\title{\bfseries Curation Over Collection: A Local-First Conversational Memory System and a Longitudinal Protocol for Evaluating It}
...
%======================================================================
\begin{abstract}
Conversational assistants forget everything between sessions, forcing users to re-establish context continually. We present the Infinite Context Engine (ICE), a local-first conversational memory system that sits between the user and any OpenAI-compatible model, maintaining persistent memory across arbitrarily long histories through episodic, knowledge-graph, procedural, and document stores with fused, decay-weighted retrieval under a per-query token budget. Evaluating such a system is itself an open problem: static benchmarks cannot capture memory that accumulates, decays, and is revised over months. We therefore introduce LSREP, a Longitudinal State-Replay Evaluation Protocol that replays real, self-authored conversations (1{,}985 turns; 1{,}211 probes across 50 checkpoints), reconstructs full memory state at each checkpoint, and scores answers against an evolving ground truth. Under LSREP, ICE matches a strong vector-RAG baseline on answer quality (paired $\Delta = +0.00$, 95\% CI $[-0.07, +0.07]$) while injecting 32\% fewer fragments, and wins a materially higher share of blind head-to-head comparisons (30.6\% vs.\ 21.2\%, non-overlapping CIs); on an independently hand-scored subset the margin is larger ($+1.21$, $[+0.81, +1.61]$). ICE's retrieved fragments correlate \emph{positively} with answer quality ($r = +0.19$) where the baseline's correlate \emph{negatively} ($r = -0.02$). Under extreme per-turn context density the unbudgeted baseline collapses (94.2\% failure) while the budgeted system holds (mean 4.33). A cumulative ablation isolates rank fusion as the mechanism that makes multi-signal retrieval safe: adding an unfused lexical signal is harmful ($-0.74$, $[-1.14, -0.36]$), and fusion recovers it ($+0.82$, $[+0.39, +1.24]$). Together these results support one principle for long-horizon conversational memory---\emph{curation, not collection}: less-but-better context, governed by a per-query budget, beats retrieving more. We release the system, the protocol, and a component-level fidelity audit of which mechanisms were active during evaluation.
\end{abstract}
