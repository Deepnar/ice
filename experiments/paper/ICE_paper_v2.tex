...
\begin{abstract}
A conversational memory system is defined by behaviours that unfold over time---accumulation, decay, and the revision of superseded beliefs---and existing benchmarks cannot observe any of them. LoCoMo and LongMemEval, the strongest instruments available, supply a fixed history up front and ask their questions at the end; nothing ages between one question and the next, so what they measure with precision is retrieval over a long transcript, not the evolution of a memory state. This paper's primary contribution is an instrument for the missing regime. We introduce \textbf{LSREP}, a Longitudinal State-Replay Evaluation Protocol, which freezes a real conversational history at many temporal checkpoints, reconstructs the complete memory state as it existed at each one, and scores answers against a ground truth that is itself allowed to evolve---so that a system which correctly \emph{updates} a fact is not penalised against one repeating an outdated one. LSREP is system-agnostic: it requires only a chat endpoint and state reconstructible from a turn sequence, and it ingests any user's exported history, which makes it reproducible on corpora we cannot ourselves release. As the system under test we present the Infinite Context Engine (ICE), a local-first conversational memory system that sits between the user and any OpenAI-compatible model and maintains persistent memory across arbitrarily long histories: episodic turns, a temporally-versioned knowledge graph, procedural patterns, and documents, retrieved through weighted rank fusion under a per-query token budget. Instantiated on 1{,}985 turns with 1{,}211 probes at 50 checkpoints, LSREP shows that ICE matches a strong vector-RAG baseline on answer quality (paired $\Delta = +0.00$, 95\% CI $[-0.07, +0.07]$) while injecting 32\% fewer fragments, and wins a materially larger share of blind head-to-head comparisons (30.6\% vs.\ 21.2\%, non-overlapping intervals); on an independently hand-scored slice the margin is $+1.21$ $[+0.81, +1.61]$. ICE's retrieved fragments correlate \emph{positively} with answer quality ($r = +0.19$) where the baseline's correlate \emph{negatively} ($r = -0.02$). Under extreme per-turn context density the unbudgeted baseline collapses (94.2\% failure) while the budgeted system holds (mean 4.33). A cumulative ablation isolates rank fusion as the mechanism that makes multi-signal retrieval safe: an unfused lexical signal is harmful ($-0.74$, $[-1.14, -0.36]$) and fusion recovers it ($+0.82$, $[+0.39, +1.24]$). We pair these results with a component-level fidelity audit that separates three things systems evaluations routinely conflate: one component that was genuinely defective (a procedural-retrieval binding bug), several that this benchmark never exercised (document retrieval, batch summarisation, temporal and cross-conversation queries), and the mechanisms that demonstrably carried the result---the token budget, rank fusion, decay-weighted episodic retrieval, and post-fusion curation. Separating ``broken'' from ``untested'' is, we argue, a methodological requirement for evaluating any multi-component memory system, and the evidence here supports one design principle: memory is curation, not collection.
\end{abstract}
...
A language model holds a conversation only for as long as its context window holds the conversation; everything older is silently gone, and the interface gives no warning that it happened. Enlarging the window does not dissolve the problem, because capacity is not use: models attend unevenly across a long context and degrade sharply on information placed away from its edges~\citep{liu2024lost}, and a model's \emph{effective} context is routinely far shorter than its advertised one~\citep{hsieh2024ruler}. We call the resulting failure \emph{context collapse}. Its clearest form occurs during creative or narrative work and in long technical discussions. The model usually retains the general idea of what is going on, but forgets specificity: the exact rule that was set, a particular character, a precise decision made yesterday, the dependency-injection pattern established three months ago. The conversation continues, but the model has quietly lost the thread. The workarounds---exporting chats, pasting into a fresh context, re-explaining everything for the third time---survive the first few exchanges of a new session and then degrade, because the assistant that started coherent is, by turn fifteen, an assistant that has lost the new context too.

This is not a minor inconvenience. For a person doing sustained work---building a software system over months, constructing an original fictional universe across hundreds of sessions, planning a multi-year academic trajectory---the lack of persistent, intelligent, user-controlled memory is the single largest bottleneck between the human and the model working as genuine collaborators. Persistence across sessions is not a new concern: dialogue research has studied it for years under multi-session chat~\citep{xu2022msc,jang2023chronicles}, more recent benchmarks push the horizon to hundreds of turns and dozens of sessions~\citep{maharana2024locomo,wu2025longmemeval}, and a parallel line of work asks how a model should be adapted to an individual user at all~\citep{salemi2024lamp}. Commercial assistants now ship cross-session memory of their own. What remains open is the \emph{shape} of what persists, and who governs it: the systems designed to address it either cap at a few hundred lines of static instructions, go stale, or live in someone else's cloud with no user control over what is remembered and what is forgotten.
...
A crucial architectural assumption is that the language model is a \emph{stateless reasoning engine}. ICE does not embed memory into weights through fine-tuning; it externalises memory entirely, storing, maintaining, and retrieving context as an independent system surrounding a model that processes whatever context it is given and forgets it immediately. Externalising memory this way has precedent in agent architectures that maintain and retrieve their own experience streams~\citep{park2023generative,packer2023memgpt}. This separation lets models be swapped or upgraded without losing memory, and makes the store transparent, inspectable, and user-controllable. A vector-RAG baseline~\citep{lewis2020rag,karpukhin2020dpr} shares the superficial property of feeding external context to a stateless model, but is not \emph{designed} to reconstruct a structured, evolving cognitive state: it stores raw chunks, with no knowledge graph, behavioural-pattern inference, or decay-and-reinforcement lifecycle.

Evaluating a system like this is a second, separable problem. A memory system's defining behaviours---accumulation, decay, revision of superseded beliefs---are invisible to a benchmark that asks questions once, at one moment, against a fixed ground truth. The strongest current benchmarks for conversational memory are built that way by design: LoCoMo~\citep{maharana2024locomo} and LongMemEval~\citep{wu2025longmemeval} supply a history up front and pose their questions at the end, which measures retrieval over a long transcript rather than the evolution of a memory state \emph{between} questions. That is the right instrument for the question they ask, and the wrong one for ours. What is needed alongside them is a protocol that can stand a system at many points along a real conversational history and ask what it knows at each of them.

This paper makes three contributions.

\begin{enumerate}[leftmargin=*]
  \item \textbf{LSREP}, a Longitudinal State-Replay Evaluation Protocol, and the paper's primary contribution. It replays months of real conversational history through a memory system, reconstructs the complete memory state at each of many temporal checkpoints, and scores answers against an \emph{evolving} ground truth. It is specified independently of ICE and depends on nothing ICE-specific: a system is admissible if it exposes a chat endpoint and its persistent state can be rebuilt from a turn sequence. Because it ingests ordinary chat-log exports, a reader can run it on their own history without access to ours.
  \item \textbf{A local-first conversational memory system} (ICE), the system under test: a multi-store architecture---episodic memory, a temporally-versioned knowledge graph, procedural memory, and document memory---with intent-driven retrieval routing, access-weighted decay, weighted rank fusion, and a dynamic per-query token budget. It runs on a single consumer GPU and is released in full. ICE is both what motivated the protocol and the first system measured under it; the protocol is not a metric fitted to this system after the fact, and Section~\ref{sec:lsrep-standing} states what would falsify that claim.
...
\textbf{Deployed commercial memory, and why it is not a baseline here.} The major hosted assistants have shipped cross-session memory features of their own, and any honest account of this area must say so plainly: it is no longer true that a conversational assistant simply forgets everything between sessions. What those features retain, however, is described in product documentation rather than in technical reports. The extraction policy, the retrieval mechanism, the update and forgetting rules, and the storage representation are all undisclosed, and no vendor exposes the memory layer through an interface that a third party could hold fixed, populate deterministically, or query in isolation from the model it serves. They therefore cannot function as a controlled baseline in this study---not because the comparison is uninteresting, but because it is not constructible. An evaluation against them would confound the memory layer with a substantially larger and differently trained answering model and with an unobservable retrieval policy, and no ablation could separate the three. We state this as a scope boundary rather than leave the omission to read as an oversight. What can be said from public behaviour is that these features present as user-visible fact stores with manual review and deletion---closer in shape to atomic-fact extraction than to a versioned graph---and that they are, by construction, the inverse of local-first: the memory lives with the vendor, on the vendor's terms.
...
None of this makes a purpose-built protocol a substitute for standard benchmarks, and we do not present it as one. The two answer different questions, as Section~\ref{sec:related-eval} sets out, and a memory system should be able to face both: an established benchmark establishes that a system is competitive on the regime the field already measures, and LSREP measures the regime it does not. We regard reporting a system on both as the correct standard, including for this system, and Section~\ref{sec:future-work} states where that stands.
