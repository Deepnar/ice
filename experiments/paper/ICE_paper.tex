...
\title{\bfseries The Infinite Context Engine: A Self-Hosted Memory Middleware with Intent-Driven Retrieval and a Temporally-Versioned Knowledge Graph}
...
A crucial architectural assumption underlying ICE is that the language model itself should be treated as a stateless reasoning engine. The model has no memory of past interactions, nor does it need to. ICE does not attempt to embed memory into the model's weights through fine-tuning or continual learning; instead, it externalises memory entirely, storing, maintaining, and retrieving context as an independent system that surrounds the model. The model processes whatever context ICE supplies and forgets it immediately after generation. This separation is deliberate: it allows models to be swapped, upgraded, or replaced without losing memory, and it makes the memory system transparent, inspectable, and user-controllable. The vector-RAG baseline shares the superficial property of feeding external context to a stateless model, but it does not reconstruct a structured, evolving cognitive state. It stores raw chunks; it does not build a knowledge graph, infer behavioural patterns, or maintain memory over time through decay and reinforcement.

In this paper we make four contributions:

\begin{enumerate}[leftmargin=*]
  \item The \textbf{ICE architecture}---a multi-store memory system with intent-driven retrieval routing, a temporally-versioned knowledge graph, access-weighted decay mechanics, and a dynamic per-query token budget.
  \item \textbf{LSREP} (Longitudinal State-Replay Evaluation Protocol)---a benchmark motivated by the failure of static precision-based metrics, in which months of conversational history are replayed through the system and memory quality is measured at multiple temporal checkpoints.
  \item \textbf{Empirical results} across four long-form conversations showing ICE matches vector-RAG quality on answer correctness while injecting 32\% fewer fragments and winning a substantially larger share of head-to-head comparisons. On a context-overflow stress test where the vector baseline collapses to a 94.2\% failure rate, ICE maintains a mean score of 4.33.
  \item A \textbf{feature-buildup ablation} identifying Reciprocal Rank Fusion as the single most impactful architectural component (+0.84 score delta) and revealing that several intuitively helpful features---MERA, HyDE, dynamic budget, cluster restriction---are neutral or negative on creative-writing data.
\end{enumerate}
...
To achieve resilient memory retrieval, ICE synthesises several foundational algorithmic techniques. \textbf{BM25} \citep{robertson2009bm25} remains one of the most successful sparse retrieval algorithms, scoring document relevance based on a non-linear term frequency function and inverse document frequency, with document length normalisation. While dense semantic embeddings excel at capturing conceptual similarity, they frequently suffer from catastrophic failures in exact-keyword matching. BM25 ensures high-fidelity lexical recall and serves as a secondary leg in ICE's multi-signal retrieval strategy. \textbf{Reciprocal Rank Fusion} \citep{cormack2009rrf} is the mathematically elegant algorithm that combines document rankings by sorting them based on the inverse of their rank positions across multiple independent search systems, preventing any single retrieval methodology from disproportionately skewing the results. RRF is ICE's indispensable fusion backbone, and our own ablation confirms that it is the single most critical mathematical component for retrieval stability. \textbf{HyDE} \citep{gao2023hyde} addresses the vocabulary mismatch problem by generating a hypothetical document from the query and embedding that instead, but our ablation shows it to be largely neutral in creative domains, indicating that its utility must be dynamically gated.
...
The mean-score difference between ICE Generalist and Vector Generalist is +0.01 (4.26 vs 4.25)---within noise---but several secondary metrics tell a more interesting story. ICE injects 32.0\% fewer fragments (20.4 vs 30.0) while achieving the same quality, giving an SPF of 0.21 vs 0.14 (a 50\% improvement in efficiency per fragment). The head-to-head win rate is the cleanest signal: ICE Generalist wins 30.6\% of probes against the vector baseline's 21.2\%---when the systems disagree, ICE is more likely to be right. ICE uses 6.6\% more tokens (22{,}411 vs 21{,}025) because its fragments are higher-quality and longer on average, but it uses them in a more compact set. Hallucination rates are roughly equal (19.6\% vs 20.5\%).

\textbf{Leg contributions} (ICE Generalist): episodic 62.3\%, codex 3.3\%, unknown (fragments whose source leg could not be reconstructed) 34.5\%. The codex contribution is small but targeted. \textbf{Fragment--score correlation} reveals a qualitative difference: ICE Generalist shows $r = +0.193$ (MoE: $+0.246$), while Vector Generalist shows $r = -0.015$ (MoE: $-0.030$). For ICE, retrieving more fragments correlates with better answers; for the vector baseline, retrieving more fragments correlates with worse answers---a strong signal that ICE's fragments carry signal while the vector baseline's carry noise.
...
\centering
\small
\caption{Cumulative feature addition on Dataset B (67 probes). RRF (+0.84) is the single most impactful step; BM25 alone ($-0.75$) is the single most destructive. Several intuitively helpful features---HyDE, cluster restriction, session diversification, MERA---are neutral or negative. Full ICE finishes $+0.11$ above bare vector but $2\times$ more efficient per fragment. The vector baseline row is included for reference (it is not a step in the buildup).}
\label{tab:ablation}
\resizebox{\textwidth}{!}{%
...
Several findings stand out. \textbf{RRF is the single most impactful component} ($+0.84$). Adding RRF after BM25 rescues the system from the BM25 damage and pushes it above the bare-vector baseline. The mechanism is clear: BM25 adds noisy lexical matches that need the fusion quality signal of RRF to be useful. \textbf{BM25 alone is destructive} ($-0.75$). Adding BM25 without RRF floods the context with low-quality lexical matches that actively hurt answer quality. This is a cautionary finding: naive multi-leg retrieval without fusion is worse than single-leg retrieval.

\textbf{HyDE, cluster restriction, and session diversification are neutral} ($\Delta \leq 0.03$). These features do not hurt, but they do not help either---at least not on a creative-writing dataset. They may help in other domains. \textbf{MERA is actively harmful} ($-0.21$). The Meta-Enumeration Retrieval Agent injects category-level entity lists that add noise without improving answer quality. On a creative-writing dataset, the user rarely asks ``list all the characters''---they ask specific questions about specific characters. \textbf{Codex has a small positive effect} ($+0.03$) but a \emph{negative} recency delta ($-0.14$), meaning Codex helps early-turn questions (by providing structured entity context from the beginning) but does not improve over time. \textbf{Dynamic budget is negative} ($-0.11$) on this dataset: the budget formula allocates more tokens to longer conversations, which \emph{hurts} on Dataset B because it injects too many fragments (21.2 vs 11.3 at the pre-budget step). This is counter-intuitive and suggests that the budget formula needs domain-specific tuning.
...
The full-ICE configuration finishes 3.38, slightly above the bare-vector baseline of 3.27 but slightly below the vector-baseline reference of 3.42. The single-domain nature of this experiment (Dataset B only, 67 probes, smaller 14B model) limits what the absolute numbers can tell us, but the relative deltas are valid: RRF matters, BM25 is dangerous without fusion, MERA hurts, and several intuitively-helpful features are neutral.

\textbf{Lessons learned.} Not all features are created equal. RRF is the linchpin: without it, multi-leg retrieval is actively harmful. Several ``obvious'' improvements (HyDE, MERA, dynamic budget) are neutral or negative, which is a humbling reminder that intuition is a poor guide in system design. The right way to build a retrieval system is not to throw every feature at the wall, but to carefully measure each one's contribution and keep only what helps.
...
\end{axis}
\end{tikzpicture}
\caption{Feature ablation buildup (Dataset B, 67 probes, Experiment 3). Each bar is the cumulative mean score after adding one feature to all previous ones. The single largest positive swing is $+$RRF ($+0.84$, from 2.52 to 3.36); the single largest negative is $+$BM25 alone ($-0.75$) and $+$MERA ($-0.21$). The dashed line marks the single-leg vector baseline (3.42), which is not a step in the buildup. RRF is the dominant single feature.}
\label{fig:ablation-waterfall}
\end{figure}

\subsection{Comparative Summary}

Across the four experiments, the story is not ``ICE is always better''---it is ``ICE is more resilient and more efficient, and its advantage grows with conversation difficulty.'' Experiment 1 showed ICE could be worse than the baseline; Experiment 2 showed it can match quality with 32\% fewer fragments; the ICE-Dev stress test showed it can survive where the baseline collapses; the ablation showed \emph{why} it works (RRF) and where it does not (MERA, untuned dynamic budget, NER-broken Codex). The narrative is one of progressive understanding rather than single-result triumph.
...
Memory is not just retrieval---it is \emph{curation}. ICE's advantage comes not from retrieving more (it retrieves less) but from retrieving better. The token budget, the RRF fusion, the decay mechanics, the cluster scoping, the session diversification, and the keyword boost all serve to filter noise. A memory system that remembers everything is a memory system that remembers nothing useful. The fragment--score correlation is the cleanest empirical statement of this principle: for ICE, more fragments correlate positively with quality ($r = +0.19$); for the vector baseline, more fragments correlate negatively with quality ($r = -0.02$). Same metric, opposite sign.

\subsection{Why Do Some Features Help in One Domain but Not Another?}

The ablation revealed that several ``obviously helpful'' features are neutral or negative on creative-writing data. MERA is designed for categorical queries (``list all characters'') that are common in technical domains but rare in creative writing. Cluster restriction assumes topical coherence that breaks down in meandering creative conversations. Dynamic budget assumes that longer conversations need more retrieval, which is true for technical planning but false for creative writing (where recent turns are often the most important). The lesson: retrieval features are \emph{domain-specific}, and a good system must adapt its feature set to the conversation type---which is exactly what ICE's classification engine attempts to do through dynamic leg weighting. The classification-driven routing is, in our view, the most under-explored design lever in the system.

\subsection{How Do the Findings Generalise?}

The token-budget finding (ICE-Dev stress test) should generalise to any system that injects retrieved context into a fixed-context-window model. The RRF finding (ablation) should generalise to any multi-leg retrieval system. The domain-specificity finding (MERA, HyDE neutral or negative in creative writing) is a caution against one-size-fits-all feature sets. The fragment--score correlation finding is, we suspect, universal: a system whose fragments correlate negatively with answer quality is, in a deep sense, a system that does not know what it is doing.
...
\textbf{Implications.} The marginal contribution of Codex should be read as a floor, not a ceiling. That it produced any positive contribution despite oversized chunks, slow corroboration, and an underpowered extractor suggests that a properly implemented Codex could become one of the strongest components in the architecture. The negative result for MERA ($-0.21$) is similarly contextual: MERA activates only when entity extraction finds no entities, so it is a downstream indicator of extraction gaps rather than an independent subsystem evaluation.
...
The limitations above define a concrete agenda. We group planned work into extraction and grounding, autonomous maintenance, cross-conversation memory, retrieval under stress, explainability, learned routing, and conversation import.

\subsection{Self-Correcting Codex Extraction}
The current Codex is a passive triplet collector. We plan a four-stage self-correcting pipeline: (1)~\emph{deterministic NER grounding} on CPU, reusing the NER model in post-flight to produce a confirmed entity list and bypass LLM hallucination for entity identification; (2)~\emph{relationship mapping} on GPU, where the LLM receives the text \emph{and} the confirmed entity list and only maps relationships between those entities, drastically reducing hallucinations like \texttt{fastapi uses fastapi}; (3)~\emph{deterministic validation}, where every proposed triplet is checked against the existing graph and contradictions trigger a reconciliation loop rather than a blind write, turning the extractor into an active state reconciler; and (4)~\emph{confidence-calibrated storage}, where edges carry a continuous extraction confidence rather than a binary \texttt{pending}/\texttt{active} flag. For code-heavy conversations, a deterministic static-analysis layer (AST parsing, import-graph construction, call tracking) will produce edges (\texttt{imports}, \texttt{calls}, \texttt{inherits}, \texttt{defined\_in}) requiring no LLM extraction at all.

\subsection{Agentic Background Maintenance}
Current background workers run fixed pipelines and cannot react to cross-cutting inconsistencies. We plan to replace individual extraction workers with a \emph{Memory Maintenance Agent}---a lightweight LLM given a toolset (\texttt{update\_entity\_relation}, \texttt{merge\_conflicting\_entities}, \texttt{reconcile\_graph\_state}, \texttt{flag\_for\_review}, \texttt{run\_cluster\_consolidation}) and tasked with maintaining memory consistency during idle GPU time. The agent would decide autonomously whether to extract, check for contradictions, merge entities, or escalate to the review queue, shifting the background worker from a deterministic script executor to an autonomous decision-maker while preserving the review queue as a safety net. The currently skeletal Sentinel system would be fully integrated, with the agent subscribing to Sentinel events and proactively resolving flagged issues.

\subsection{Cross-Conversation Project-State Memory and Coding Mode}
We plan to extend ICE beyond single-conversation memory via a \emph{Project State Engine}: dedicated tables and workers tracking architecture clusters, decisions, tasks, and git history alongside the conversational stores. Each project maintains its own memory scope; retrieval operates across a project's entire history under a per-project token budget. This enables coding-mode retrieval (surfacing relevant code files and architecture decisions across all project conversations), decision tracking (critical decisions become Codex edges with explicit temporal provenance), git integration (each commit treated as a timestamped fact scanned by a state-reconciler agent), and task management (the \texttt{pending\_items} slot elevated to a full task table with status tracking).

\subsection{Retrieval Under Extreme Context Density}
The ICE-Dev stress test motivates retrieval methods for extremely dense contexts: \emph{chunk-aware retrieval} that operates at the 512-token chunk level rather than the turn level, preventing a single 8{,}000-token document from dominating the budget; \emph{hierarchical summarisation} producing a three-layer abstraction (100-token abstract, 500-token summary, full document) so retrieval surfaces the cheapest sufficient layer; \emph{document decomposition} into section-level chunks with independent embeddings; and \emph{context-adaptive truncation} that prioritises relevant chunks over whole documents, replacing the current heuristic truncation with a learned policy.

\subsection{Unified, User-Controlled Context Budgeting}
The dynamic token budget is currently opaque. We plan to expose it through the frontend: a retrieval-aggressiveness slider (capped / moderate / aggressive), a force-wide-net ``deep search'' toggle, per-project budget overrides, and a real-time telemetry panel showing how budget was allocated across legs for the current query. This turns the budget from a hidden heuristic into a user-controlled resource and creates a natural sandbox for learning optimal policies from user feedback.

\subsection{User-Defined Scoping and Cross-Conversation Linking}
The scoping system (\texttt{None}/\texttt{Auto}/\texttt{Project}/\texttt{Manual}) will be extended with temporary retrieval scopes (marking a turn, cluster, or document as relevant for the next $N$ queries), manual and suggested conversation linking (based on entity overlap and topic continuity), cross-project entity relevance, and selective memory exclusion---all user-controlled, preserving user authority over memory while letting the system assist with relevance inference.

\subsection{Learned Mixture-of-Experts Routing}
The hardcoded overlap scorer will be replaced with a learned policy incorporating classifier confidence, context-reliance labels, and empirical model-performance data. Candidate approaches include confidence-weighted routing, context-reliance-aware routing (\texttt{Zero\_Shot} to smaller faster models, \texttt{Long\_Term\_Memory} to larger ones), a lightweight bandit tracking per-topic model performance, and model-loading integration that pre-loads the selected model to eliminate the 5--15~s switch latency. The LSREP protocol can itself be repurposed to learn these policies offline by replaying historical probes under different routing policies and measuring answer quality.

\subsection{Long-Horizon Studies, Explainability, and KV-Cache Persistence}
Three further directions follow directly from the limitations. \emph{Year-scale deployments} will study memory saturation, retrieval drift, long-term decay dynamics, and compaction frequency beyond the current 93-day horizon. An \emph{explainability (forensics) layer} will expose retrieval attribution (which leg produced each fragment and why), memory provenance (every fragment traceable to its origin turn via \texttt{source\_batch\_id}), a visual Codex graph with editable edges, and queryable audit trails---essential for trust in a system that evolves autonomously. \emph{KV-cache persistence} will investigate storing precomputed KV tensors for stable prefix segments on disk, cache-aware retrieval policies that prefer already-cached fragments, and incremental cache updates, as inference engines expose more cache-control APIs.

\subsection{User-Guided Continual Learning}
An opt-in feedback system will let users rate responses: a thumbs-up adds the query and classification to the curated dataset; a thumbs-down triggers re-classification by a stronger model, with the corrected label added to the dataset. The weekly fine-tune worker consumes this dataset and retrains the classifier head, and a promotion script closes the currently manual loop by swapping the checkpoint and restarting the proxy. A manual-correction side panel lets experienced users edit predicted labels directly. The system is disabled by default, with explicit opt-in and warnings about the risk of degrading the classifier with inconsistent feedback.

\subsection{Conversation Import as a First-Class Feature}
LSREP, developed as a research benchmark, is in effect a complete conversation-ingestion pipeline: the same chronological replay, post-flight evaluation, Codex extraction, procedural extraction, clustering, and reflection used for evaluation can ingest exported chat logs. This directly attacks the vendor lock-in of memory---users with months or years of history on cloud platforms could export it, drop it into an ingestion endpoint, and obtain a fully mature ICE deployment that treats the imported history as if it had always been present. A key design lever is decay scaling for imported history: rather than applying months of decay on arrival, ingested turns receive a configurable stabilisation window (a \texttt{decay\_immune} flag), an optional fast-forward mode that simulates the original timeline, and user-selectable ingestion policies (preserve all / simulate natural decay / start fresh). Because ICE reconstructs episodic memory, a knowledge graph, procedural memory, and clusters as distinct stores, import means reconstructing the entire cognitive state the system would have had if present during the original conversation---not merely building a searchable archive, as a naive vector-only import would. The imported history is not just retrievable text; it is \emph{lived through} by the background workers, producing the same structured memory state that would have emerged from live interaction.

%======================================================================
\section{Conclusion}
\label{sec:conclusion}

We have presented the Infinite Context Engine, a self-hosted memory middleware with a multi-store architecture, intent-driven retrieval routing, a temporally-versioned knowledge graph, and a per-query dynamic token budget. We have presented LSREP, a longitudinal evaluation protocol that emerged from the failure of a precision-based metric and that measures not just retrieval quality at a single point in time, but memory accumulation, retention, and belief revision across months of conversational history. We have presented four experiments that, taken together, show ICE matches vector-RAG quality on long-form conversations while injecting 32\% fewer fragments, wins a substantially larger share of head-to-head comparisons, and survives a context-overflow stress test where the vector baseline collapses to a 94.2\% failure rate. We have presented a feature-buildup ablation that identifies RRF as the single most impactful component, that reveals several intuitively-helpful features to be neutral or negative, and that gives a quantitative roadmap for future development.

As context windows grow, the temptation is to simply stuff more context in. Our results suggest the opposite: less-but-better context outperforms more-but-noisier context in long-horizon conversational AI. The future of conversational memory is not bigger windows---it is smarter curation. ICE's architecture, with its classification-driven routing, RRF-based fusion, and per-query token budget, points toward a design philosophy where the memory system acts as a \emph{curator} rather than a \emph{firehose}. We hope this work contributes to that philosophy and that the released implementation, evaluation protocol, and ablation harness enable others to build on it.

%======================================================================
\section*{Broader Impact and Reproducibility}

The substantive limitations of this work are enumerated in full in Section~\ref{sec:limitations}: single-user evaluation, synthetic longitudinal reconstruction, classifier-override conservatism, Codex extraction fragility, the neutrality of the MoE router, ineffective KV-caching in practice, the Experiment 3 audit gap, and constrained domain and hardware coverage. We add two further observations on reproducibility and ethics.

\textbf{Reproducibility.} The full source code, the evaluation harness, and the experimental configuration are released as part of this work. All reported results reflect the system as it existed at the evaluation-time snapshot; the implementation has continued to evolve since, and the repository tags the exact evaluated revision. All numbers reported in Section~\ref{sec:results} are derived from publicly shareable JSON reports; the raw judge-model outputs and the LSREP probe sets are not released because they contain personal conversational content, but the experimental configuration files are sufficient to regenerate the benchmark on any comparable conversation corpus.
