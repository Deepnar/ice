...
# =====================
# Reads master_results.json and curation files, then runs all judge passes for every probe.
# Uses SGLang Gemma-12B as judge. Saves raw judge outputs per probe to evaluation_raw.json.
# """
...
# # ---------------------------------------------------------------------------
# # Added "Dossier vs Answer" clarification and prioritized Detail over Fluff.
...
# Judge based on whether the conversational response accurately reflects the facts in the dossier.

# Rate the AI's answer on a scale of 1 to 5 based on:
# - Accuracy: Does it match the ground truth facts exactly?
# - Completeness: Does it include all relevant details from the dossier?
# - Relevance: Does it stay focused on the user's question?
# - Detail: High-resolution factual accuracy is the priority. Do not penalize for length or "fluff" if the information is grounded in the dossier.
...
# # Added "Dossier vs Answer" clarification.
...
# # Added "Lore-only" restriction to prevent false flags on general knowledge.
...
# Identify how much "noise" (irrelevant or off-topic text) is present relative to the user's question.
...
#         "probe_id": probe_id,
#         "question": question,
#         "ground_truth": ground_truth,  # store truncated for sanity
#         "absolute_scores": {},
#         "tournament": None,
#         "hallucination": {},
#         "fragment_analysis": {},
...
#     # -----------------------------------------------------------------------
#     # 1. Absolute Scoring (each condition)
#     # -----------------------------------------------------------------------
...
#         resp = await call_judge(session, messages, sem)
#         if resp:
#             try:
...
#             try:
#                 parsed = json.loads(resp)
#                 # Map labels back to original condition names
