...
| `test_codex_2_0.py` | **archived 2026-07-28.** Three independent disqualifications at once: (1) **broken** — crashed at `orchestrator._load_ner_model()`, a method deleted when NER moved to `retrieval/ner_utils.py`; (2) **destructive** — opened with `TRUNCATE episodic_memory, conversations, codex_* … RESTART IDENTITY CASCADE` on the live DB, the same criterion that retired `test_full_pipeline_phase_7.py`; (3) two of its four sections were print-only or dead (extraction needs a live bg model; MERA was deleted by A4 and its section had degraded to an ImportError skip). | **none — deliberately not replaced yet.** See the ⚠ note below. |
